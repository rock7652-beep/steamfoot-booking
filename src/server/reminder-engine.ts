...
export interface TriggeredBooking {
  booking: BookingWithCustomer;
  triggerAt: Date;
}

/** findTriggeredBookings 只用到的規則欄位 — 收緊型別以利 dashboard 重用 */
export type ReminderRuleForTrigger = {
  type: string;
...
/**
 * 找出指定規則在 [windowStart, windowEnd) 內應觸發的預約。
 * 公開給 dashboard stats 使用，計算「今日尚未發送」數量。
 */
export async function findTriggeredBookings(
  rule: ReminderRuleForTrigger,
  windowStart: Date,
  windowEnd: Date,
