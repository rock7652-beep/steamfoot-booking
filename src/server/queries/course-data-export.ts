...
export async function getCourseDataExport(storeId: string, type: DataExportType, period: { gte: Date; lte: Date }, status: string | undefined, limit: number, options: { now?: Date; revenueStaffId?: string } = {}): Promise<Sheet[]> {
  ...
  if (type === "transactions") {
    const [orders, refunds, trialPayments] = await Promise.all([
      ...
      coursePrisma.courseTrialPayment.findMany({where:{storeId,createdAt:period,...(options.revenueStaffId ? {actorUserId:{in:(await prisma.staff.findMany({where:{storeId,id:options.revenueStaffId},select:{userId:true}})).map(s=>s.userId)}} : {}),...(status ? {status:status==="VOIDED"?"VOIDED":status==="CONFIRMED"?"SUCCESS":"NO_MATCH"}: {})},include:{booking:{select:{customerId:true,status:true,session:{select:{nameSnapshot:true,startsAt:true}}}}},orderBy:{createdAt:"desc"},take}),
    ]);
    ...
    return [
      {name:"體驗收款",headers:["收款編號","收款時間","顧客","課程","上課時間","實收","付款方式","收款狀態","出席狀態","更正原因"],rows:trialPayments.map(p=>[p.id,stamp(p.createdAt),names.get(p.booking.customerId)??"顧客待核對",p.booking.session.nameSnapshot,stamp(p.booking.session.startsAt),p.amount,({CASH:"現金",TRANSFER:"轉帳",LINE_PAY:"LINE Pay",CREDIT_CARD:"信用卡",OTHER:"其他"} as Record<string,string>)[p.paymentMethod]??p.paymentMethod,p.status==="SUCCESS"?"已收款":"已作廢",courseExportStatusLabel("bookings",p.booking.status),p.voidReason??""])},
