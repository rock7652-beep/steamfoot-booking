...
export async function courseHomeAccess(user: User, storeId: string) {
    const [bookings, create, attendance, customers, updateCustomers, revenue, confirm, cash, blocked] = await Promise.all([
    ...
    return { bookings, create: create && bookings && writable, customers, revenue, cash, staffScope: courseCustomerStaffScope(user, storeId), todos: { payments: revenue && confirm && writable, attendance: bookings && attendance && writable, followUp: customers && updateCustomers && writable, staffScope: courseCustomerStaffScope(user, storeId) } };
}
