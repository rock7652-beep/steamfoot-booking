...
export async function getCourseBusinessAnalytics(storeId: string, range: CourseAnalysisRange, scope: BusinessScope, access: { money: boolean; customers: boolean; fees: boolean }) {
  ...
  const [customers, staff, records, store] = await Promise.all([
  ...
  const coverageStart=toLocalDateStr(new Date(Math.min(+store.createdAt,...records.sessions.map(s=>+s.startsAt),...records.purchases.filter(p=>p.confirmedAt).map(p=>+p.confirmedAt!),...records.receipts.map(r=>+r.createdAt))));
  ...
  if(access.money&&scope.view!=="coach") {
    ...
    for(const row of [...(retailHistory?.daily??[]), ...(retailHistory?.otherDaily??[])])addMoney(new Date(`${row.date}T00:00:00.000Z`),row.revenue);
  }
