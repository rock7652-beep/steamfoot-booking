...
/**
 * Fixed, non-sensitive stages for the immutable flow publish transaction.
 * The underlying database error is deliberately discarded so callers can log
...
export type DigitalButlerPublishStageCode =
  ...
  | "ACTIVE_VERSION_UPDATE_FAILED"
  | "AUDIT_CREATE_FAILED"
  | "TRANSACTION_COMMIT_FAILED";

export class DigitalButlerPublishStageError extends Error {
  constructor(readonly code: DigitalButlerPublishStageCode) {
    super(`DIGITAL_BUTLER_PUBLISH_${code}`);
    this.name = "DigitalButlerPublishStageError";
  }
}
...
async function publishStage<T>(
  ...
  try {
    ...
    throw new DigitalButlerPublishStageError(code);
  }
}
...
export class DigitalButlerRepository {
  ...
  async publishFlow(input: PublishDigitalButlerFlowInput) {
    ...
    try {
      ...
      const lock = await publishStage(input.diagnosticStages === true, "FLOW_LOCK_FAILED", () => tx.storeDigitalButlerFlow.updateMany({
      ...
      const draftUpdate = input.draftUpdate;
      if (draftUpdate) {
        await publishStage(input.diagnosticStages === true, "DRAFT_UPDATE_FAILED", () => tx.storeDigitalButlerFlow.update({
          where: { id_storeId: { id: input.flowId, storeId: input.storeId } },
          data: { name: draftUpdate.name, draftDefinition: draftUpdate.definition },
        }));
      }
      const latest = await publishStage(input.diagnosticStages === true, "VERSION_ALLOCATION_FAILED", () => tx.digitalButlerFlowVersion.aggregate({
        where: { flowId: input.flowId, storeId: input.storeId },
        _max: { version: true },
      }));
      const version = await publishStage(input.diagnosticStages === true, "VERSION_AND_STEPS_CREATE_FAILED", () => tx.digitalButlerFlowVersion.create({
      ...
      await publishStage(input.diagnosticStages === true, "ACTIVE_VERSION_UPDATE_FAILED", () => tx.storeDigitalButlerFlow.update({
        where: { id_storeId: { id: input.flowId, storeId: input.storeId } },
        data: {
          status: "PUBLISHED",
          enabled: true,
      ...
      if (audit) {
        await publishStage(input.diagnosticStages === true, "AUDIT_CREATE_FAILED", () => tx.auditLog.create({
          data: {
