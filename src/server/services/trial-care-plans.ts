...
export async function publicPlanMessages(storeId: string, token: string, page: number): Promise<LineMessage[]> {
  const store = await prisma.store.findFirst({ where: { id: storeId, operatingStatus: "ACTIVE" }, select: { name: true, industryModule: true } });
  if (!store) return [{ type: "text", text: "目前無法提供本店方案，請直接回覆訊息聯繫店家。" }];
  const paging = { skip: page * PAGE_SIZE, take: PAGE_SIZE + 1 };
  let plans: { name: string; price: unknown; count: number; validityDays: number | null; description?: string | null }[] = [];
  if (store.industryModule === "STEAMFOOT") {
    const rows = await prisma.servicePlan.findMany({ where: { storeId, isActive: true, publicVisible: true, category: "PACKAGE" }, orderBy: [{ sortOrder: "asc" }, { id: "asc" }], ...paging, select: { name: true, price: true, sessionCount: true, validityDays: true, description: true } });
    ...
    const rows = await spaPrisma.spaPackage.findMany({ where: { storeId, isActive: true, publicVisible: true, treatmentId: { in: treatments.map(t => t.id) } }, orderBy: [{ name: "asc" }, { id: "asc" }], ...paging, select: { name: true, price: true, uses: true, validityDays: true } });
