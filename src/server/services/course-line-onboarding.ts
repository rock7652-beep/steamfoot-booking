...
export async function onboardCourseLineMember(input: {
  ...
  if (await resolveVerifiedLineCustomer(input.storeId, input.lineUserId)) return { status: "ok" };
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const candidates = await tx.customer.findMany({
        where: { storeId: input.storeId, OR: [{ phone }, { lineUserId: input.lineUserId }] },
        select: { id: true }, take: 2,
      });
      const links = await tx.customerIdentityLink.findMany({
        where: { storeId: input.storeId, OR: [
          { provider: "line", providerAccountId: input.lineUserId },
      ...
      if (!account) await tx.account.create({ data: {
        userId, provider: "line", providerAccountId: input.lineUserId, type: "oauth",
      } });
      const customer = await tx.customer.create({ data: {
        ...
        authSource: "LINE", lineUserId: input.lineUserId,
      ...
      await tx.customerIdentityLink.create({ data: {
        ...
        provider: "line", providerAccountId: input.lineUserId, lineUserId: input.lineUserId,
      } });
