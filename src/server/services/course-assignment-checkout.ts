...
export async function assignCourseWithCheckout(tx:Prisma.TransactionClient,actor:{storeId:string;userId:string},data:CourseCheckoutInput & {planId:string;customerId:string;expiresDate:string;requestKey:string}) {
  ...
  const card=await tx.coursePointCard.create({data:{storeId,planId:plan.id,nameSnapshot:plan.name,unit:plan.unit,templateIds:plan.templateIds,remaining:plan.points,expiresAt,requestKey:data.requestKey,members:{create:{customerId:data.customerId}},entries:{create:{kind:"GRANT",points:plan.points,actorUserId:userId}}}});
  ...
  const order=await tx.coursePurchase.create({data:{storeId,customerId:data.customerId,planId:plan.id,name:plan.name,unit:plan.unit,points:plan.points,price:total.paid,validDays:plan.validDays,templateIds:plan.templateIds,status:"CONFIRMED",transferLastFive:"",requestKey:data.requestKey,cardId:card.id,confirmedAt:new Date(),confirmedBy:userId,note,listPrice:total.listPrice,discountKind:data.discountKind,discountValue:data.discountValue,paymentMethod:method,transferLastFour:lastFour}});
  if(total.paid>0) await tx.$executeRaw`INSERT INTO "CashbookEntry" (id,"storeId","entryDate",type,"paymentMethod",category,amount,note,"createdByUserId","updatedAt") VALUES (${"course-purchase:"+order.id},${storeId},${day},'INCOME',${method==="CASH"?"CASH":"OTHER"}::"CashbookPaymentMethod",'課程方案',${total.paid},${plan.name+" / "+note},${userId},NOW())`;
