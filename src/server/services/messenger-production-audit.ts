...
export type MessengerAuditSafeResult = {
...
function requiredConfigNames(storeSlug: string): string[] {
...
function getAuditConfig(storeSlug: string): AuditConfig {
  const page = getMessengerPageConfig(storeSlug);
  if (requiredConfigNames(storeSlug).length > 0 || !page.pageId || !page.accessToken) {
    throw new Error("audit_configuration_incomplete");
  }
  return {
...
/** Read-only Meta control-plane audit. Credentials and raw Graph bodies remain inside this module. */
export async function runMessengerProductionAudit(storeSlug: string): Promise<MessengerAuditSafeResult> {
  const config = getAuditConfig(storeSlug);
...
export async function createMessengerAuditRun(input: { storeId: string; storeSlug: string; requestedByUserId: string }) {
  const run = await prisma.messengerAuditRun.create({
    data: { storeId: input.storeId, requestedByUserId: input.requestedByUserId },
  });

  try {
    const result = await runMessengerProductionAudit(input.storeSlug);
    const hasGraphFailure = Object.values(result.calls).some((call) => !call.ok);
    return await prisma.messengerAuditRun.update({
      where: { id: run.id },
      data: {
        completedAt: new Date(),
        status: hasGraphFailure ? "COMPLETED_WITH_ERRORS" : "COMPLETED",
        appValidated: result.appValidated,
        pageTokenMatches: result.pageTokenMatches,
        callbackMatches: result.callbackMatches,
        configuredFields: result.configuredFields,
        missingFields: result.missingFields,
        pageAttached: result.pageAttached,
        callsSafeSummary: result.calls,
        errorCode: hasGraphFailure ? "GRAPH_REQUEST_FAILED" : null,
      },
    });
  } catch (error) {
    const errorCode = error instanceof Error && error.message === "audit_configuration_incomplete"
      ? "AUDIT_CONFIGURATION_INCOMPLETE"
      : "AUDIT_UNAVAILABLE";
    return prisma.messengerAuditRun.update({
      where: { id: run.id },
      data: { completedAt: new Date(), status: "FAILED", errorCode },
    });
  }
}
