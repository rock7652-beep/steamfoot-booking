...
async function requireSpaCheckoutStore() {
  ...
  if ((await prisma.storeModuleInstallation.findUnique({where:{storeId}}))?.status !== "ACTIVE") throw new AppError("FORBIDDEN", "此店尚未完成服務模組設定");
...
export async function settleSpaBookingWithPayment(input: z.infer<typeof paymentSchema>): Promise<ActionResult<{ bookingId: string; serviceCompleted: boolean }>> {
  try {
    ...
    await spaPrisma.$transaction(async (tx) => {
      ...
      const current = await tx.spaBooking.findFirst({where:{id:booking.id,storeId}});
      if (!current || !["PENDING","CONFIRMED"].includes(current.status) || await tx.spaReceipt.findFirst({where:{bookingId:booking.id,storeId}})) throw new AppError("CONFLICT", "此預約已結帳或取消，請重新整理");
