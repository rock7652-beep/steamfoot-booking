...
function failed(e: unknown) {
  ...
  return { success: false as const, error: r.success ? "操作失敗" : r.error };
}
...
export async function getSpaCustomerAccount(customerId: string) {
  try {
    ...
    const [packages, wallets, entitlements, sales, receipts, refunds, entries] =
      await Promise.all([
        spaPrisma.spaPackage.findMany({
          where: { storeId, isActive: true },
          orderBy: { name: "asc" },
        }),
        spaPrisma.$queryRaw<
          { id: string; balance: Prisma.Decimal; status: string }[]
        >`SELECT id,balance,status FROM "SpaStoredValueWallet" WHERE "storeId"=${storeId} AND "customerId"=${customerId}`,
        spaPrisma.$queryRaw<
          {
            id: string;
            name: string;
        ...
        >`SELECT id,"entryType"::text,amount,"balanceAfter","createdAt" FROM "SpaStoredValueEntry" WHERE "storeId"=${storeId} AND "customerId"=${customerId} ORDER BY "createdAt" DESC LIMIT 100`,
      ]);
    return {
      ...
      wallet: wallets[0]
        ? { ...wallets[0], balance: Number(wallets[0].balance) }
        : null,
      ...
      sales: sales.map((s) => ({
        ...
        refunded: refunds.some((r) => r.saleId === s.id),
      })),
      receipts: receipts.map((r) => ({
        id: r.id,
        name: r.booking.serviceNameSnapshot,
        amount: Number(r.amount),
        paymentMethod: r.paymentMethod,
        transferLast4: r.transferLast4,
        uses: r.uses,
        createdAt: r.paidAt.toISOString(),
        refunded: refunds.some((f) => f.receiptId === r.id),
      })),
...
const saleSchema = z
  .object({
    ...
    kind: z.enum(["PACKAGE", "TOPUP"]),
    packageId: z.string().optional(),
    expectedPackageUpdatedAt: z.string().datetime().optional(),
  ...
  .refine(
    validSpaTransferReference,
    "轉帳請填寫轉出帳號後四碼，其他付款方式不需填寫",
  );
export async function purchaseSpaCredit(input: z.infer<typeof saleSchema>) {
  try {
    const user = await requirePermission("transaction.create"),
      storeId = await activeStore("wallet.create"),
      d = saleSchema.parse(input);
    const fingerprint = JSON.stringify(d);
    const sale = await spaPrisma.$transaction(
      async (tx) => {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`spa-schedule:${storeId}`},0))`;
        const previous = await tx.spaCreditSale.findUnique({
          where: { storeId_requestKey: { storeId, requestKey: d.requestKey } },
        });
        if (previous) {
          if (previous.fingerprint !== fingerprint)
            throw new AppError("CONFLICT", "這筆送出內容已變更，請重新開啟");
          return previous;
        }
        if (
          !(await prisma.customer.findFirst({
            where: { id: d.customerId, storeId },
            select: { id: true },
          }))
        )
        ...
        let name: string;
        if (d.kind === "PACKAGE") {
          const p = await tx.spaPackage.findFirst({
            where: { id: d.packageId ?? "", storeId, isActive: true },
          });
          if (
            !p ||
            p.updatedAt.toISOString() !== d.expectedPackageUpdatedAt ||
            Number(p.price) !== d.amount
          )
            throw new AppError("CONFLICT", "方案內容或價格已變更，請重新選擇");
          if (
            !(await tx.spaTreatment.findFirst({
              where: { id: p.treatmentId, storeId, isActive: true },
            }))
          )
            throw new AppError("CONFLICT", "方案服務已停用");
          sourceId = randomUUID();
          name = p.name;
          const start = parseTaiwanDateToDbDate(toLocalDateStr()),
            expiry = new Date(start);
          expiry.setUTCDate(expiry.getUTCDate() + p.validityDays - 1);
          ...
          if (d.amount <= 0)
            throw new AppError("VALIDATION", "儲值金額必須大於零");
          name = "儲值加值";
          const wallets = await tx.$queryRaw<
            { id: string; balance: Prisma.Decimal }[]
          >`INSERT INTO "SpaStoredValueWallet" (id,"storeId","customerId",balance,"updatedAt") VALUES(${randomUUID()},${storeId},${d.customerId},${d.amount},CURRENT_TIMESTAMP) ON CONFLICT ("storeId","customerId") DO UPDATE SET balance="SpaStoredValueWallet".balance+EXCLUDED.balance,"updatedAt"=CURRENT_TIMESTAMP WHERE "SpaStoredValueWallet".status='ACTIVE' RETURNING id,balance`;
          if (!wallets[0]) throw new AppError("CONFLICT", "儲值帳戶已停用");
          sourceId = wallets[0].id;
        ...
        return tx.spaCreditSale.create({
          data: {
            id: saleId,
            storeId,
            ...
            ...(d.transferLast4 ? { transferLast4: d.transferLast4 } : {}),
            sourceId,
...
export async function refundSpaPayment(input: z.infer<typeof refundSchema>) {
  try {
    ...
    const refund = await spaPrisma.$transaction(
      async (tx) => {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`spa-schedule:${storeId}`},0))`;
        const previous = await tx.spaRefund.findFirst({
          where: {
            storeId,
        ...
        if (d.kind === "SALE") {
          ...
          if (paymentMethod === "STORED_VALUE") {
            ...
            >`UPDATE "SpaEntitlementUse" SET status='RELEASED',"releasedAt"=CURRENT_TIMESTAMP WHERE "entitlementId"=${receipt.sourceId} AND "storeId"=${storeId} AND "bookingId"=${receipt.bookingId} AND status='COMPLETED' AND uses=${uses} RETURNING id,uses`;
            if (!usage[0])
              throw new AppError("CONFLICT", "原方案扣次紀錄不一致，尚未退回");
