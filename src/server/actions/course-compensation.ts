...
export async function saveCourseCompensation(input:unknown){try{
 ...
 await courseTransaction(storeId,async tx=>{
  ...
  if(d.staffId){
   ...
   if(!staff.length)throw new AppError("FORBIDDEN","請先儲存這位教練的授課資格");
   if(d.rules.length!==1)throw new AppError("VALIDATION","教練每個授課項目請選一種計酬方式");

  }else if(rows.some(r=>r.staffId&&r.rules.some(rule=>!d.rules.some(v=>v.mode===rule.mode))))throw new AppError("CONFLICT","有教練正在使用要移除的計酬方式，請先調整教練設定");
