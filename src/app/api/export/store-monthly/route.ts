...
export async function GET(req: NextRequest) {
  ...
  if (searchParams.has("startDate") || searchParams.has("endDate")) {
    ...
    const rows = await analysisExportRows(activeStoreId, searchParams, false);
    return new NextResponse("\uFEFF" + toCsv(rows), { headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="store-analysis-${start}-${end}.csv"`, "Cache-Control": "no-store" } });
  }
