...
export function TrialBookingManager() {
  ...
  useEffect(() => {
    if (disabled) return;
    void getTrialBookingManagementStatusFromChat(token).then(setBooking).catch(() => setMessage("無法取得預約資訊，請聯絡門市。"));
  }, [disabled, token]);
  ...
  useEffect(() => {
    ...
    if (action === "reschedule") void loadRescheduleSlots();
  ...
  const cancelButton = <button disabled={disabled || cancelled} className="mt-4 w-full rounded-xl border border-red-300 p-3 text-red-700 disabled:opacity-40" onClick={() => { if (window.confirm("確定要取消這筆預約嗎？")) void cancelTrialBookingFromChat(token).then(result => {
    if (result === "cancelled" || result === "already_cancelled") setBooking(current => current ? { ...current, bookingStatus: "CANCELLED" } : current);
  ...
  const rescheduleSection = <section className="mt-4 rounded-xl bg-white p-4">
    ...
    <input className="mt-3 w-full rounded border p-2" type="date" value={date} onChange={e => {
      ...
      setSlots([]);
      setSelected("");
      void getTrialRescheduleSlotsFromChat(token, e.target.value).then(nextSlots => setSlots(nextSlots)).catch(() => setMessage("無法取得可改期時段。"));
    }} />
  ...
  return <main className="mx-auto min-h-dvh max-w-lg bg-[#f7f2ea] p-6 text-earth-900">
    ...
    {cancelled ? null : action === "cancel" ? cancelButton : action === "reschedule" ? rescheduleSection : action === "confirm" ? null : <>
