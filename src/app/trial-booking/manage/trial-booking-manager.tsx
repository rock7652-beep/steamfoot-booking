...
export function TrialBookingManager() {
  ...
  const cancelButton = <button disabled={disabled} className="mt-4 w-full rounded-xl border border-red-300 p-3 text-red-700 disabled:opacity-40" onClick={() => { if (window.confirm("確定要取消這筆預約嗎？")) void cancelTrialBookingFromChat(token).then(result => setMessage(result === "cancelled" ? "預約已取消，名額已釋出。" : result === "already_cancelled" ? "這筆預約已取消。" : "此預約目前無法自行取消，請聯絡門市。")); }}>確認取消預約</button>;
