...
export function StaffWorkScreen({ storeName, storeSlug, liffId, today }: { storeName: string; storeSlug: string; liffId: string; today: string }) {
  ...
  return <main className="mx-auto min-h-screen max-w-md bg-[#f8f5ee] px-4 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] text-earth-900">
    <header><p className="text-xs font-semibold tracking-[0.12em] text-primary-700">{storeName}</p><div className="mt-2 flex items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">我的工作</h1><p className="mt-1 text-sm text-earth-500">{state.staffName}</p></div><Link href={`/s/${storeSlug}/liff`} onClick={() => localStorage.setItem(`spa-member-mode:${storeSlug}`, "member")} className="rounded-full border border-earth-300 bg-white px-4 py-2 text-sm font-medium">會員專區</Link></div></header>
