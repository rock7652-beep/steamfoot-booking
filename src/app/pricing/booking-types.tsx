...
export function BookingTypes({ compact = false }: { compact?: boolean }) {
  return <section id="how-it-works" aria-labelledby="booking-types-title" className={compact ? "my-8 scroll-mt-24" : "mx-auto max-w-6xl scroll-mt-24 px-5 pb-10 pt-4 sm:px-8"}>
    ...
    <div className="mt-5 grid gap-3 md:grid-cols-3">
      {[
        { name: "時段預約", question: "幾點還能接幾位？", who: "蒸足、岩盤浴、桑拿、親子遊戲等固定場次消費", href: "/pricing/features/slots", cta: "看時段預約", status: "按時段接待顧客" },
      ...
      ].map((item, i) => <article key={item.name} className="flex flex-col rounded-2xl border border-[#153B31]/15 bg-white/75 p-4 sm:p-6">
        {i === 2 ? <p className="mb-2 text-sm font-medium text-[#80602E]">{item.status}</p> : null}
        <h3 className="text-xl font-semibold">{item.name}</h3>
        <p className="mt-2 text-base font-medium">{item.question}</p>
        <p className="mt-1 text-sm leading-6 text-[#4C6259]">{item.who}</p>
        <Link href={item.href} className="mt-2 inline-flex min-h-11 items-center font-semibold underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-4">{item.cta}<span aria-hidden="true" className="ml-2">→</span></Link>
      </article>)}
    </div>
