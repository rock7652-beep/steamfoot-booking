...
export default async function CustomerLayout({
  ...
  if (industryModule === "course" && !["/profile", "/member-stores", "/member-link"].some(p => pathname.startsWith(p))) {
    ...
    return <div className="min-h-screen bg-earth-50"><header className="flex items-center justify-between border-b bg-white px-4 py-1 text-sm"><span>{customerFacingStoreName}</span><form action={logoutAction}><input type="hidden" name="storeSlug" value={storeCtx.storeSlug} /><button className="min-h-11 px-3">登出</button></form></header>{children}</div>;
  }
