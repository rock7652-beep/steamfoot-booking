      ...
      <div className="space-y-6">
        ...
        {!showOnboardingBanner && (
          <div className="rounded-2xl border border-earth-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold text-earth-900">已連結門市</h2>
            <div className="mt-4 divide-y divide-earth-200 rounded-xl border border-earth-200 px-4">
              {centralMember.memberships.map((membership) => (
                <p key={membership.storeId} className="py-4 font-medium text-earth-900">
                  {membership.storeName}
                </p>
              ))}
              {centralMember.memberships.length === 0 && (
                <p className="py-4 text-sm text-earth-600">目前尚未找到已連結門市。</p>
              )}
            </div>
