...
export default async function DashboardLayout({
  ...
  if (industryModule === "course") {
    ...
    if (!/\/dashboard\/?$/.test(requestedPath) && !/\/dashboard\/(?:courses(?:\/|$)|duty(?:\/\d{4}-\d{2}-\d{2})?\/?$|settings\/duty\/?$|staff(?:\/[^/]+\/edit)?\/?$|cashbook(?:\/new|\/[^/]+\/edit)?\/?$|cash-drawer\/?$|revenue\/?$|reconciliation\/?$|store-revenue\/?$|guide\/?$)/.test(requestedPath)) {
