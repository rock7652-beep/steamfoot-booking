...
import { CourseMonthlySettings,CourseMonthlyConfirm,CourseProfitPay,CourseProfitCorrect } from "./course-monthly-client";
...
export async function CourseMonthly({storeId,month,readOnly=false}:{storeId:string;month:string;readOnly?:boolean}){
 ...
 return <PageShell><PageHeader title="課程月結管理" subtitle={`${store?.name??"本店"} · ${month}`} actions={<MonthFilter month={month}/>}/>
 <KpiStrip items={[{label:"目前應付",value:money(total),tone:"primary"},{label:"已登錄付款",value:money(paid)},{label:"待付",value:money(payable)},{label:"溢付待核對",value:money(overpaid),tone:overpaid?"amber":"primary"}]}/>
 ...
 {!confirmed&&!readOnly&&<CourseMonthlyConfirm key={report.fingerprint} month={month} fingerprint={report.fingerprint} revision={last?.revision??0} disabled={issues>0||!report.lines.length}/>}
 <section className="space-y-3" aria-label="人員月結清單">
 {!people.length&&<p className="rounded-lg border p-6">此月份沒有需結算的利潤或授課費。</p>}
 {people.map(person=><details key={person.id} className="rounded-xl border border-earth-200 bg-white p-4"><summary className="cursor-pointer list-none"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{person.name}</strong><span className="text-sm text-primary-700">展開明細 ＋</span></div><dl className="mt-3 grid grid-cols-2 gap-3 text-sm md:grid-cols-5">{[["店長利潤",person.profit],["授課費",person.fee],["合計應付",person.profit+person.fee],["已付",person.paid],[person.remaining<0?"溢付待核對":"淨待付",Math.abs(person.remaining)]].map(([label,value])=><div key={label}><dt className="text-earth-500">{label}</dt><dd className="font-medium">{money(Number(value))}</dd></div>)}</dl>{person.issues>0&&<p className="mt-2 text-sm text-amber-800">{person.issues} 筆需核對</p>}</summary>
 <div className="mt-4 divide-y border-t">{person.lines.map(line=><article key={line.kind+line.id} className="space-y-2 py-4 text-sm"><p className="font-medium">{line.kind==="PROFIT"?"店長利潤":"授課費"} · {line.label}</p><p>{formatTWDateTime(new Date(line.date))} · 應付 {line.amount===null?"待核對":money(line.amount)} · 已付 {money(line.paid)}</p>{line.issue&&<p role="alert" className="text-amber-800">{line.issue}</p>}{line.amount!==null&&line.paid>line.amount&&<p className="text-amber-800">溢付 {money(line.paid-line.amount)}，請核對退款／取消與實際款項。</p>}
