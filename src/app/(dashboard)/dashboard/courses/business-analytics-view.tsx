...
export function BusinessAnalyticsView({data,all,staffId}:{data:CourseBusinessReport;all:boolean;staffId?:string|null}) {
  ...
  const metricNames={attendance:"來客人次",trial:"體驗人數",newCard:coach?"體驗後新卡":"新卡人數",renewal:trendPeriod==="current"?"續卡筆數":"續卡人數",revenue:"收款淨額"};
  const unit=metric==="revenue"?"元":metric==="attendance"?"人次":metric==="renewal"&&trendPeriod==="current"?"筆":"人";
  ...
  return <div className="space-y-4" aria-busy={pending}>
    ...
    <section className="rounded-xl border border-earth-200 bg-white p-4" aria-label="趨勢分析">
      ...
      {monthly&&<><p className="mt-2 text-xs text-earth-500 lg:hidden">可左右滑動查看各月份。</p>{monthRows.some(row=>!row.available)&&<p className="mt-2 text-xs text-earth-500">尚無資料：{monthRows.filter(row=>!row.available).map(row=>row.date).join("、")}。這些月份早於建店且無歷史紀錄，不以 0 計算。</p>}</>}
    </section>
