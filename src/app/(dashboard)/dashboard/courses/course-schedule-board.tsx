...
function SessionCard({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className={`w-full rounded-md border text-left transition ${dense ? "h-full overflow-hidden px-1.5 py-0.5" : "p-2"} hover:border-primary-300 hover:bg-primary-50/40 focus:outline-none focus:ring-2 focus:ring-primary-200 ${moved ? "border-indigo-200 bg-indigo-50/80" : musicDense && recurrenceLabel === "隔週固定" ? "border-blue-200 border-l-[3px] border-l-blue-500 bg-blue-50/40" : musicDense && recurrenceLabel === "每週固定" ? "border-teal-200 border-l-[3px] border-l-teal-500 bg-teal-50/30" : musicDense && !copy.privateClass ? "border-earth-200 border-l-2 border-l-primary-300 bg-primary-50/20" : "border-earth-200 bg-white"}`}
      title={`${hhmm(session.startsAt)} ${copy.primary} · ${copy.coach} · ${copy.room}${recurrenceLabel ? ` · ${recurrenceLabel}` : ""}`}
      aria-label={`${copy.primary}，${hhmm(session.startsAt)}，${copy.coach}${recurrenceLabel ? `，${recurrenceLabel}` : ""}`}
    >
      {musicDense ? (
        <>
          <div className="flex min-w-0 items-center gap-1 text-[11px] leading-4">
            ...
            {brief ? <span className="max-w-[48%] shrink-0 truncate text-[10px] font-semibold text-earth-700">{copy.coach}</span> : recurrenceLabel && <span className={`shrink-0 rounded px-1 text-[9px] font-semibold ${recurrenceLabel === "隔週固定" ? "bg-blue-100 text-blue-800" : "bg-teal-100 text-teal-800"}`}>{recurrenceLabel === "隔週固定" ? "隔週" : "固定"}</span>}
          </div>
          {!brief && <p className="truncate text-[10px] font-semibold leading-4 text-earth-700">{copy.coach}</p>}
          {!brief && <p className="truncate text-[10px] leading-4 text-earth-500">{copy.privateClass ? session.nameSnapshot : `${session.bookings.length}/${session.capacity} 人`}{moved ? " · 調課" : ""}{isTrial(session) ? " · 體驗" : ""}</p>}
        </>
      ) : (
      <>
      ...
      <div className={`flex flex-wrap gap-1 ${dense ? "mt-1" : "mt-1.5"}`}>
        ...
        {!moved && recurrenceLabel && (
          <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${recurrenceLabel === "隔週固定" ? "bg-blue-100 text-blue-800" : "bg-teal-100 text-teal-800"}`}>{recurrenceLabel}</span>
        )}
        {isTrial(session) && (
...
export function CourseScheduleBoard({
  ...
  const recurrenceLabels = courseRecurrenceLabels(sessions);
                    ...
                    {list.length ? (
                      list.map((session) => (
                        <SessionCard
                          ...
                          recurrenceLabel={recurrenceLabels.get(session.id)}
