...
const descriptions: Record<string, string> = {
  today: "今天未取消的課次；預約排除已取消，同一學員跨堂計人次，今日人數按實際學員去重。完成為已出席人次，不是完成課次。",
    ...
    <div className="space-y-3">
      ...
      <div className="grid items-start gap-3 xl:grid-cols-2">
        {access.cash && <Stream title="開店與對帳" id="cash" load={async () => {
                ...
                return <><div className="flex items-center justify-between gap-3"><strong className="text-sm">{labels[c.state]}</strong><Link className={linkStyle} href="/dashboard/cash-drawer">管理現金 →</Link></div>{"expected" in c && <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm"><p>應有現金：{c.expected == null ? "待核對" : money(c.expected)}</p><p>結帳實點：{c.actual == null ? "尚未盤點" : money(c.actual)}</p><p>結帳差額：{c.difference == null ? "尚未確認" : money(c.difference)}</p>{c.state === "OPEN" && <p className="text-xs text-earth-500">開店實點 {money(c.openingActual)}・開店差額 {money(c.openingDifference)}。尚未結帳盤點，不代表已對平。</p>}</div>}</>;
            }}/>}
