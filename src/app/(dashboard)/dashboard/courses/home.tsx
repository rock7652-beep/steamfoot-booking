...
export async function CourseHome({ user, storeId }: {
    ...
    <div className="space-y-3">
      ...
      {access.revenue && <Stream title="今日收款" id="receipts" load={async () => { const r = await getCourseReceiptTotals(storeId, date, date); return <><Link href={revenueHref} className="flex min-h-11 flex-wrap items-center gap-x-5 gap-y-2 text-sm"><strong className="text-lg text-primary-900">{money(r.gross)}</strong><span>退款 {money(r.refunds)}</span><span>沖銷 {money(r.voids)}</span><span>淨收款 {money(r.net)}</span><span className="text-primary-700 underline">查看收款明細</span></Link><p className="text-xs text-earth-500">方案核帳＋體驗收款入帳，退款及沖銷另列；不等於現金餘額，不重複加計現金帳。</p></>; }}/>}
