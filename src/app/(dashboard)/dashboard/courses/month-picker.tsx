...
export function CourseMonthPicker({ month }: { month: string }) {
  ...
  const button =
    "min-h-11 rounded-lg border border-earth-200 bg-white px-3 disabled:opacity-50";
  return (
    <form
      className="flex flex-wrap items-center gap-3"
      onSubmit={(e) => {
        e.preventDefault();
        change(String(new FormData(e.currentTarget).get("month")));
      }}
    >
      <button
        type="button"
        ...
        disabled={pending}
        onClick={() =>
          change(addTaiwanDuration(`${month}-01`, -1, "MONTH").slice(0, 7))
        }
      >
        ‹
      </button>
