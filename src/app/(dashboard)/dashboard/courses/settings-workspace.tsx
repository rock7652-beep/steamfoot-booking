  ...
  return <SettingsPanelContext.Provider value={context}><div className="grid min-w-0 gap-4 md:grid-cols-[190px_minmax(0,1fr)]">
    ...
    <div className="min-w-0 rounded-xl border border-earth-200 bg-white px-4 py-2 sm:px-6">
      ...
      <section hidden={active !== "booking"} aria-label="營業與預約">
        ...
        <Row title="營業與公休" summary="每週營業時間與特殊日期；名額依各堂課及教室容量設定。" href={props.canHours ? "/dashboard/courses/hours?tab=weekly" : undefined} action="每週營業時間">
          {props.canHours && <DashboardLink href={courseSettingsPanelHref("/dashboard/courses/hours?tab=special")} scroll={false} className="mt-2 inline-flex min-h-11 items-center rounded-lg border px-3 text-sm text-primary-700">特殊公休／調整日期</DashboardLink>}
        </Row>
