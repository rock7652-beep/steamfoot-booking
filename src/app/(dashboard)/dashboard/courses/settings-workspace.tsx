  ...
  return <SettingsPanelContext.Provider value={context}><div className="grid min-w-0 gap-4 md:grid-cols-[190px_minmax(0,1fr)]">
    ...
    <div className="min-w-0 rounded-xl border border-earth-200 bg-white px-4 py-2 sm:px-6">
      ...
      <section hidden={active !== "store"} aria-label="店家資料">
        <Row title="店家資料" summary="名稱、地址與官方聯繫方式，統一在此維護。">{!props.canEdit && <InfoList density="compact" items={[{ label: "店家名稱", value: props.name }, { label: "地址", value: props.address || "尚未填寫" }, { label: "地圖", value: props.mapUrl ? "已設定" : "尚未設定" }, { label: "官方 LINE", value: props.lineOfficialUrl ? "已設定" : "尚未設定" }]} />} {editor({ section: "store", name: props.name, address: props.address, mapUrl: props.mapUrl, lineOfficialUrl: props.lineOfficialUrl }, props.canEdit)}</Row>
      </section>
      <section hidden={active !== "booking"} aria-label="營業與預約"><SectionGuard section="booking" context={context}>
        ...
        <Row title="營業與公休" summary="每週營業時間與特殊日期；名額依各堂課及教室容量設定。">
          {props.canHours && <div className="mt-3 flex flex-wrap gap-2"><DashboardLink href={courseSettingsPanelHref("/dashboard/courses/hours?tab=weekly")} scroll={false} className="inline-flex min-h-11 items-center rounded-lg border px-3 text-sm text-primary-700">每週營業時間</DashboardLink><DashboardLink href={courseSettingsPanelHref("/dashboard/courses/hours?tab=special")} scroll={false} className="inline-flex min-h-11 items-center rounded-lg border px-3 text-sm text-primary-700">特殊公休／調整日期</DashboardLink></div>}
        </Row>
      ...
      <section hidden={active !== "payment"} aria-label="收款與體驗"><SectionGuard section="payment" context={context}>
        {props.canPayment ? <Row title="銀行轉帳資訊" summary="填寫顧客轉帳時使用的銀行資料。">
