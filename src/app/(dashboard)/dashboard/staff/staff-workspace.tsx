...
export function StaffWorkspace({ courseBasicOnly = false, people: initialPeople, today, canManage, showSpaCompensation, createAction }: {
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-primary-200 bg-primary-50/60 px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><h2 className="text-sm font-semibold text-earth-900">{courseBasicOnly ? "教練與人員帳號" : "人員建好一次，平常只處理例外"}</h2><p className="mt-1 text-xs text-earth-600">{courseBasicOnly ? "管理基本資料與登入權限；教練的課程安排統一在課表排程操作。" : "設定專業項目與每週固定班表後，請假、臨時加班才需要再次調整。"}</p></div>
          <div className="flex gap-2">
            {!courseBasicOnly && servicePeople.length > 0 ? <button type="button" onClick={() => setEditor({ type: "exception" })} className="rounded-lg border border-primary-300 bg-white px-3.5 py-2 text-xs font-semibold text-primary-800 hover:bg-primary-50">＋ 請假／臨時加班</button> : null}
      ...
      <section className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3" aria-label="人員總覽">
        {people.map((person) => (
          <article key={person.id} className="rounded-xl border border-earth-200 bg-white p-4 shadow-sm">
            ...
            {!courseBasicOnly && <div className="mt-4 space-y-3 border-t border-earth-100 pt-3">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-earth-100 pt-3 text-xs"><span className="text-earth-400">{courseBasicOnly ? person.phone || "尚未設定電話" : `直屬顧客 ${person.customerCount} 位`}</span><button type="button" onClick={() => setEditor({ type: showSpaCompensation ? "setup" : "details", personId: person.id })} className="font-medium text-primary-700 hover:underline">{showSpaCompensation ? "查看與設定 →" : "基本資料與聯絡人 →"}</button></div>
          </article>
        ))}
      </section>

      {!courseBasicOnly && <section className="overflow-hidden rounded-xl border border-earth-200 bg-white shadow-sm">
