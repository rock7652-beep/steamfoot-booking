...
export default async function SpaStaffPage({searchParams}:{searchParams:Promise<{month?:string}>}){
 ...
 const [staff,shifts,exceptions]=await Promise.all([prisma.staff.findMany({where:{storeId,status:"ACTIVE"},select:{id:true,displayName:true},orderBy:{displayName:"asc"}}),spaPrisma.spaStaffAvailability.findMany({where:{storeId,isActive:true}}),spaPrisma.spaStaffAvailabilityException.findMany({where:{storeId,date:{gte:start,lt:end}}})]);
 return <StaffScheduleWorkspace people={staff.map(s=>({id:s.id,name:s.displayName,treatmentIds:assignments.services.filter(t=>t.staffIds.includes(s.id)).map(t=>t.id),shifts:shifts.filter(l=>l.staffId===s.id).map(l=>({dayOfWeek:l.dayOfWeek,startTime:l.startTime,endTime:l.endTime}))}))} services={assignments.services} month={month} today={today} exceptions={exceptions.map(e=>({staffId:e.staffId,date:e.date.toISOString().slice(0,10),type:e.type,startTime:e.startTime,endTime:e.endTime}))}/>;
}
