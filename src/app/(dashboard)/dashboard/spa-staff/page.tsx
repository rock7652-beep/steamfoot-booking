...
export default async function SpaStaffPage({searchParams}:{searchParams:Promise<{month?:string}>}){
 ...
 const [staff,shifts,exceptions,pendingBookings]=await Promise.all([prisma.staff.findMany({where:{storeId,status:"ACTIVE"},select:{id:true,displayName:true,isOwner:true,memberLink:{select:{revokedAt:true}},user:{select:{phone:true,status:true,passwordHash:true,email:true}}},orderBy:{displayName:"asc"}}),spaPrisma.spaStaffAvailability.findMany({where:{storeId,isActive:true}}),spaPrisma.spaStaffAvailabilityException.findMany({where:{storeId,date:{gte:start,lt:end}}}),spaPrisma.spaBooking.groupBy({by:["serviceStaffId"],where:{storeId,status:{in:["PENDING","CONFIRMED"]}},_count:{_all:true}})]);
 return <StaffScheduleWorkspace people={staff.map(s=>({id:s.id,name:s.displayName,phone:s.user.phone??"",memberLinked:!!s.memberLink&&!s.memberLink.revokedAt,editable:!s.memberLink&&!s.isOwner&&s.id.startsWith(`spa-person:${storeId}:`)&&s.user.status==="SUSPENDED"&&!s.user.passwordHash&&!s.user.email,pendingBookings:pendingBookings.find(b=>b.serviceStaffId===s.id)?._count._all??0,treatmentIds:assignments.services.filter(t=>t.staffIds.includes(s.id)).map(t=>t.id),shifts:shifts.filter(l=>l.staffId===s.id).map(l=>({dayOfWeek:l.dayOfWeek,startTime:l.startTime,endTime:l.endTime}))}))} services={assignments.services} month={month} today={today} exceptions={exceptions.map(e=>({staffId:e.staffId,date:e.date.toISOString().slice(0,10),type:e.type,startTime:e.startTime,endTime:e.endTime}))}/>;
}
