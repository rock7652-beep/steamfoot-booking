...
export function LocationWorkspace({locations,treatments}:{locations:Location[];treatments:{id:string;name:string}[]}) {
  ...
  return <main className="mx-auto max-w-6xl p-6">
    ...
    <div className="divide-y rounded-xl border bg-white">{locations.length===0?<p className="p-6">尚未設定服務位置，請新增第一個位置。</p>:locations.map(l=><button key={l.id} className="flex w-full items-center justify-between p-5 text-left hover:bg-earth-50" onClick={()=>open(l)}><div><strong>{l.name}</strong><p className="mt-1 text-sm text-earth-500">{treatments.filter(t=>l.treatmentIds.includes(t.id)).map(t=>t.name).join("、")||"尚未指定適用療程"}</p></div><span>{l.isActive?"啟用":"停用"} · 編輯</span></button>)}</div>
