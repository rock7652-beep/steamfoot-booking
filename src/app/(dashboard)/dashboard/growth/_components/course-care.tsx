...
export async function CourseCare({ storeId, month, readOnly, canFollowUp, canBook, segment, staffScope = null }: { storeId: string; month: string; readOnly: boolean; canFollowUp: boolean; canBook: boolean; segment?: string; staffScope?: string | null }) {
  ...
  const hasItems = selected ? counts[selected] > 0 : Object.values(counts).some(count => count > 0);
  return <PageShell>
    ...
    <nav aria-label="關懷分類" className="mb-4 flex flex-wrap gap-2">{([['birthday','本月生日'],['inactive','好久不見'],['low','建議回課'],['expiring','建議續約'],['trial','體驗未購買']] as const).map(([kind,label]) => <Link key={kind} aria-current={selected === kind ? "page" : undefined} className="inline-flex min-h-11 items-center rounded-lg border px-3 text-sm aria-[current=page]:bg-primary-50" href={`/dashboard/growth?segment=${kind}&month=${month}`}>{label} · {counts[kind]}</Link>)}<Link href={`/dashboard/growth?month=${month}`} className="inline-flex min-h-11 items-center rounded-lg border px-3 text-sm">全部</Link></nav>
    {!hasItems && <p className="rounded-xl border border-earth-200 bg-white p-5 text-sm text-earth-600">{selected ? "此分類目前沒有待關懷顧客。" : "目前沒有待關懷顧客。"}</p>}
    {hasItems && <div className="space-y-6">
      {(birthdayItems.length > 0 && (!selected || selected === "birthday")) && <CareSection title="本月生日" description={`${month}，每位顧客只列一次。`} emptyText="本月沒有生日顧客。" items={birthdayItems} totalCount={birthdayItems.length}/>}
