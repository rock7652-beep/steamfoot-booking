/**
 * 現金異動表單（presentational）— PR-G5.1a
 *
 ...
 *
 * 每個元件只負責 render 欄位 + 送出鈕；server action（含 redirect glue）由
 * 各 caller 自己用 closure 綁好 sessionId / returnPath 後當 `action` prop 傳入。
 * 真正的業務邏輯仍在 addCashDrawerEntryAction（單一來源），本檔不重算任何東西。
 */
...
/** 提領：現金從抽屜拿出去（不算店內支出）。 */
export function WithdrawalForm({ action }: { action: EntryAction }) {
  return (
    <form action={action} className="space-y-3 p-4 md:space-y-4">
...
/** 補入：補找零金 / 備用金 / 保險箱補現金。 */
export function DepositForm({ action }: { action: EntryAction }) {
  return (
    <form action={action} className="space-y-3 p-4 md:space-y-4">
      <div className="md:grid md:grid-cols-2 md:gap-4">
        <div>
...
export function AdjustmentForm({ action }: { action: EntryAction }) {
  return (
    <form action={action} className="space-y-3 p-4 md:space-y-4">
      <div>
