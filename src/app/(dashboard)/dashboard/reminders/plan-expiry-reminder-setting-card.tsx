    ...
    <details className="group rounded-xl border border-earth-200 bg-white shadow-sm">
      ...
      <div className="border-t border-earth-100 p-4">
        <LineCardPreview title="方案提醒" subtitle="方案將於 14 天後到期" actions={[{ label: course?"查看方案與期限":"立即預約" }, { label: "諮詢店長", variant: "link" }]}>
          <p className="font-semibold">王小美 您好</p><p>方案名稱　{course?"課程點數方案":"蒸足保養 5 堂"}</p><p>{course?"剩餘 5 點／占用 3 點／可用 2 點":"剩餘堂數　2 堂"}</p><p>方案到期日　2026/09/30</p><p>課程需於方案有效期限內完成，預約日期不可晚於到期日。</p>
        </LineCardPreview>
        <p className="mt-3 text-xs leading-relaxed text-earth-500">示意資料。{course?"通知以同店仍具共卡授權的成員為對象，點／堂分開，方案到期日變更會重新核對；每位成員於每個到期階段只成功送達一次。":"卡片會自動帶入顧客姓名、方案、剩餘堂數與到期日；同一方案在每個提醒階段只發送一次。"}</p>
      </div>
    </details>
  );
}
