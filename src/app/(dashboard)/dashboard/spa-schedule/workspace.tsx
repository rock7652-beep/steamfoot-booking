...
export function SpaScheduleWorkspace(props: Props) {
  ...
  return <>
    ...
    {!staff.length ? <p className="rounded-xl border p-8">尚無可安排的服務人員。<Link className="ml-2 underline" href="/dashboard/spa-staff">新增人員與排班 →</Link></p> :
      <div className="min-w-0 max-w-full max-h-[70dvh] overscroll-contain overflow-auto rounded-xl border border-earth-200 bg-white" ref={node => { if (node && node.dataset.positioned !== "yes") { node.scrollTop = 9 * 60 / interval * rowHeight; node.dataset.positioned = "yes"; } }}>
    ...
    {draft && <RightSheet open onClose={() => { if (!pending) setDraft(null); }} width={520} labelledById="spa-panel-title">
      <header className="flex shrink-0 items-center justify-between border-b p-5"><h2 id="spa-panel-title" className="text-xl font-bold">{editing ? "預約詳情" : "新增預約"}</h2><button disabled={pending} onClick={() => setDraft(null)} aria-label="關閉預約面板">✕</button></header>
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5">
        ...
        <nav aria-label="預約步驟" className="mb-5 grid grid-cols-4 gap-1">{["服務", "時間", "顧客", "確認"].map((label, i) => <button key={label} disabled={pending || confirmCancel} onClick={() => setStep(i)} className={`min-h-11 rounded-lg py-2 text-sm ${i === step ? "bg-earth-800 text-white" : "bg-earth-50"}`}>{i + 1} {label}</button>)}</nav>
      ...
      <footer className="flex shrink-0 flex-wrap items-center gap-2 border-t p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] [&>button]:min-h-11">
        {!confirmCancel&&editing&&editable&&canCheckout&&<button disabled={pending} className="rounded-lg bg-teal-800 px-3 py-2 text-white" onClick={()=>{setCheckout(editing);setDraft(null);}}>完成並結帳</button>}
        {editing && editable && (confirmCancel ? <div className="w-full rounded-lg border border-red-200 p-3" role="group" aria-label="取消預約確認"><p className="mb-2 text-sm">確認取消這筆預約？取消後將釋放人員與服務位置時段。</p><button disabled={pending} onClick={()=>submit(true)} className="mr-2 min-h-11 rounded-lg bg-red-700 px-3 py-2 text-white">確認取消預約</button><button disabled={pending} onClick={()=>setConfirmCancel(false)} className="min-h-11 rounded-lg border px-3 py-2">保留預約</button></div> : <button disabled={pending} onClick={() => setConfirmCancel(true)} className="mr-auto rounded-lg border border-red-200 px-3 py-2 text-red-700">取消預約</button>)}
        ...
        {!confirmCancel && step > 0 && <button disabled={pending} onClick={() => setStep(step - 1)} className="rounded-lg border px-3 py-2">上一步</button>}
        {!confirmCancel && (step < 3 ? <button disabled={pending || (editable && (step===0 ? !draft.treatmentIds.length : step===1 ? checkingProviders || !effectiveLocation || !availableProviders.some(p=>p.id===draft.serviceStaffId) : !draft.customerId))} onClick={() => setStep(step + 1)} className="rounded-lg bg-earth-800 px-4 py-2 text-white disabled:opacity-50">下一步</button>
