...
export function CourseAssignmentPayment({price,canDiscount,storeCost=0,showAllocation=false,onSummary}:{
  ...
  return <section className="min-w-0 space-y-3" aria-labelledby="assignment-payment-title">
    ...
    {paid!==null&&paid<storeCost&&<p role="alert" className="text-sm text-red-700">實收低於店家成本，請核對優惠；尚不能結帳。</p>}
    {showAllocation&&paid!==null&&paid>=storeCost&&<p className="text-sm text-earth-600">店家成本 NT$ {storeCost.toLocaleString()} · 開發人所得 NT$ {calculateCourseSaleAllocation(paid,storeCost).developerAmount.toLocaleString()}</p>}
