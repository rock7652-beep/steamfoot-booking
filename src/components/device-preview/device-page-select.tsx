...
import { getDevicePreviewPages } from "@/lib/device-preview";
...
export function DevicePageSelect({ value, onChange, moduleId = "steamfoot" }: DevicePageSelectProps) {
  return (
    <label className="flex min-w-0 flex-1 flex-col gap-1.5 text-xs font-medium text-earth-600 sm:max-w-xs">
      ...
      <select
        ...
        aria-label="快速前往"
      >
        {getDevicePreviewPages(moduleId).map((page) => (
