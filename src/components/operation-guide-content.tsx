...
export function OperationGuideContent({ full = false }: { full?: boolean }) {
  const searchId = useId();
  const [query, setQuery] = useState("");
  const [articleId, setArticleId] = useState<string | null>(null);
  const [expanded, setExpanded] = useState(full);
  const article = bookingGuides.find((item) => item.id === articleId);
  const results = searchBookingGuides(query);

  return <div className="space-y-5">
    <p className="text-sm text-earth-500">時段預約 · 目前提供 3 篇操作教學</p>
    {article ? <>
      <button type="button" onClick={() => { setArticleId(null); setExpanded(full); }} className="min-h-11 text-sm font-semibold text-primary-800">← 返回問題列表</button>
      ...
      <button type="button" aria-expanded={expanded} onClick={() => setExpanded(!expanded)} className="min-h-11 font-semibold text-primary-800 underline underline-offset-4">{expanded ? "收起完整說明" : "查看完整說明"}</button>
      {expanded && <>
      ...
      <h2 className="text-xl font-semibold text-primary-900">你想處理什麼？</h2>
      ...
      <div className="space-y-3">{results.map((item) => <button key={item.id} type="button" onClick={() => { setArticleId(item.id); setExpanded(full); }} className="flex min-h-14 w-full items-center justify-between gap-3 rounded-xl border border-earth-200 bg-white p-4 text-left font-medium text-primary-900 hover:border-gold-400"><span>{item.title}</span><span aria-hidden="true">›</span></button>)}</div>
