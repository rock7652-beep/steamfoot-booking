...
export function OperationGuideShell({ enabled, children }: { enabled: boolean; children?: ReactNode }) {
  ...
  return <GuideContext.Provider value={{ open, register }}>
    <div data-operation-guide-shell data-guide-open={opened ? "true" : "false"}>
      {children}
      <dialog ref={panel} aria-labelledby={heading} onCancel={close}
        onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); close(); } }}
        className="fixed inset-x-0 bottom-0 top-auto z-[80] m-0 max-h-[85dvh] w-full max-w-none overflow-hidden rounded-t-2xl border border-gold-200 bg-earth-50 p-0 text-earth-900 shadow-xl backdrop:bg-black/30 lg:bottom-0 lg:left-auto lg:right-0 lg:top-14 lg:h-[calc(100dvh-3.5rem)] lg:max-h-none lg:w-[380px] lg:rounded-none">
        <div className="flex h-16 items-center justify-between border-b border-gold-200 px-5">
        ...
        <div data-guide-scroll className="max-h-[calc(85dvh-4rem)] space-y-5 overflow-y-auto p-5 lg:h-[calc(100%-4rem)] lg:max-h-none">
          <button type="button" onClick={() => { setSession((n) => n + 1); panel.current?.querySelector("[data-guide-scroll]")?.scrollTo({ top: 0 }); }} className="min-h-11 text-sm font-semibold text-primary-800 underline underline-offset-4">查看此頁相關問題</button>
          <OperationGuideContent key={session} context={bookingPage ? (status ? "booking-detail" : "booking-list") : "general"} bookingStatus={status} />
