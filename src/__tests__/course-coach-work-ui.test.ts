...
describe("member plan and purchase navigation", () => {
  ...
  it("shows the same personal consumption ledger from member and coach modes", async () => {
    ...
    const role = host.querySelector('select[aria-label="身分"]') as HTMLSelectElement;
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, "value")!.set!.call(role, "coach");
