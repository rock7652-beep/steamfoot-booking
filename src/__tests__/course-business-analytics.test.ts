...
describe("course business analytics",()=>{
 ...
 it("counts music teacher share for both seats including an absent group student",()=>{const s=session("s1","2026-09-02");s.bookings.push({...s.bookings[0],customerId:"absent",status:"NO_SHOW"});const r=summarizeCourseBusiness({...data,sessions:[s],fees:[{sessionId:"s1",staffId:"c1",rule:{mode:"SHARE",value:60},musicPricePerLesson:650,musicTeacherFeeBase:null}]});expect(r.fee).toBe(780);expect(r.missingFees).toBe(0);});
