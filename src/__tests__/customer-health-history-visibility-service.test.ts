...
describe("manager-visible customer health history", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(["ADMIN", "OWNER", "PARTNER"])(
    "lets %s read exact verified cross-store memberships without customer consent",
    async (staffRole) => {
      const memberships = [
        {
          storeId: "store-a",
  ...
  it("keeps non-manager roles on the target store only", async () => {
    const result = await getStaffVisibleHealthSummary({
      staffRole: "STAFF",
      targetStoreId: "store-b",
      targetCustomerId: "customer-b",
    ...
    expect(result.hasCrossStoreAccess).toBe(false);
    expect(h.localSummary).toHaveBeenCalledWith("customer-b", "store-b");
    expect(h.resolveOwner).not.toHaveBeenCalled();
    expect(h.crossStoreSummary).not.toHaveBeenCalled();
  });

  it("fails closed when the target store membership is not verified", async () => {
  ...
  it("fails closed when central identity cannot be resolved", async () => {
    h.resolveOwner.mockResolvedValue({ status: "identity_conflict" });
