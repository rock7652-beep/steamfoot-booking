...
it("keeps edits across pagination, search and collapse and saves hidden edits", async () => {
  expect(host.querySelectorAll('button[aria-expanded]')).toHaveLength(10);
  await click("編輯"); await input("方案0 提醒門檻", "7"); await click("下一頁"); await click("編輯"); await input("方案10 提醒門檻", "8");
  await input("搜尋提醒方案", "方案20"); expect(host.querySelectorAll('button[aria-expanded]')).toHaveLength(1);
...
it("validates hidden drafts before saving and confirms discard", async () => {
  await click("編輯"); await input("方案0 提醒門檻", ""); await click("下一頁"); await click("儲存全部修改"); expect(mocks.save).not.toHaveBeenCalled(); expect(host.textContent).toContain("方案0");
