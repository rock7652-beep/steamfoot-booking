...
vi.mock("@/lib/spa-db", () => ({ spaPrisma: { spaBooking: { findMany: bookingFindMany } } }));
...
describe("LIFF staff work projection", () => {
  it("reads only the linked staff and exact store", async () => {
    ...
    expect(result).toMatchObject({ status: "ok", staffName: "小美", rows: [{ customerName: "測試顧客", locationName: "美容床 1" }] });
