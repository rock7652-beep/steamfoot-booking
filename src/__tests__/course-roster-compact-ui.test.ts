...
it("shows ten compact rows and selects all twenty for one batch without cancelled bookings",async()=>{
 ...
 try {
  ...
  expect(host.querySelectorAll('input[aria-label^="選取 "]')).toHaveLength(10);
