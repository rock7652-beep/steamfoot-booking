...
describe("settings store-scope source contracts", () => {
  ...
  it("scopes payment and trial writes through the shared resolver", () => {
    ...
    const payment = source.slice(
      source.indexOf("export async function updateShopBankInfo"),
    );
    const trial = source.slice(
      source.indexOf("export async function updateTrialSettings"),
    );
  ...
  it("remounts store-scoped setting forms when the active store changes", () => {
    ...
    const trialPage = read(
      "src/app/(dashboard)/dashboard/settings/trial/page.tsx",
    );
    const referralPage = read(
      "src/app/(dashboard)/dashboard/settings/referral-share/page.tsx",
    );
    const hoursPage = read(
      "src/app/(dashboard)/dashboard/settings/hours/page.tsx",
    );
    const dutyPage = read(
      "src/app/(dashboard)/dashboard/settings/duty/page.tsx",
    );
    const remindersPage = read(
