...
describe("Taichung LINE OAuth session bridge", () => {
  ...
  it("rejects a tampered or expired signed cookie", async () => {
    ...
    const raw = issueTaichungLineSession({ attemptId: "a", userId: "u", customerId: "c", storeId: "s" });
  ...
  it("retains the exact customer, store, and user claims for server-side binding checks", async () => {
    const { issueTaichungLineSession, verifyTaichungLineSession } = await import("@/lib/line-oauth/taichung-session");
    const raw = issueTaichungLineSession({ attemptId: "attempt", userId: "user-taichung", customerId: "customer-taichung", storeId: "store-taichung" });
    expect(verifyTaichungLineSession(raw)).toMatchObject({ userId: "user-taichung", customerId: "customer-taichung", storeId: "store-taichung" });
  });
});
