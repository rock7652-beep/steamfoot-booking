...
describe("Messenger production audit", () => {
  ...
  it("logs one safe structured result while returning the same safe body", async () => {
    configure();
    ...
    const route = await import("@/app/api/internal/messenger-production-audit/route");
    const response = await route.POST(new Request("https://test/api/internal/messenger-production-audit", {
      method: "POST", headers: { Authorization: "Bearer audit-token-secret" },
    }));
    const body = await response.json();
    const logged = log.mock.calls.find(([event]) => event === "MESSENGER_AUDIT_RESULT");
    const serialized = JSON.stringify({ body, logged });

    expect(response.status).toBe(200);
    expect(logged).toHaveLength(2);
    expect(logged?.[1]).toMatchObject({ appValidated: true, pageTokenMatches: true, callbackMatches: true, pageAttached: true, missingFields: [] });
    expect(serialized).not.toContain(secretValues.MESSENGER_APP_ACCESS_TOKEN);
    expect(serialized).not.toContain(secretValues.MESSENGER_PAGE_ACCESS_TOKEN_ZHUBEI);
