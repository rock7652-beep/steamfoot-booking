...
describe("customer records authorization and pagination", () => {
 ...
 it("scopes consumption records and clamps invalid page input", async () => {
  ...
  expect(mocks.payments).toHaveBeenCalledWith(expect.objectContaining({ where: { customerId: "customer-a", storeId: "store-a" }, take: 30 }));
  expect(mocks.cashbook).toHaveBeenCalledWith(expect.objectContaining({ where: { customerId: "customer-a", storeId: "store-a", type: "INCOME" }, take: 30 }));
