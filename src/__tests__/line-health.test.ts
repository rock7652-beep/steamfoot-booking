...
describe("Taichung OA token health check", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    h.permission.mockResolvedValue({ role: "OWNER", storeId: "store-taichung" });
    h.activeStore.mockResolvedValue("store-taichung");
    h.store.mockResolvedValue({ lineDestination: "Ustored-bot-id" });
  ...
  it("does not call LINE for a PARTNER even if a permission check succeeds", async () => {
    h.permission.mockResolvedValue({ role: "PARTNER", storeId: "store-taichung" });
    await expect(checkTaichungLineBotHealth()).resolves.toMatchObject({ success: false });
  ...
  it("fails closed when the store destination or token check is unavailable", async () => {
    h.store.mockResolvedValue({ lineDestination: null });
