...
const m=vi.hoisted(()=>({store:vi.fn(),tx:vi.fn(),staffFind:vi.fn(),staffList:vi.fn(),staffCount:vi.fn(),treatments:vi.fn(),links:vi.fn(),regular:vi.fn(),exceptions:vi.fn(),bookings:vi.fn(),locations:vi.fn()}));
...
vi.mock("@/lib/spa-db",()=>({spaPrisma:{$transaction:m.tx,spaTreatment:{findMany:m.treatments},spaStaffSkill:{findMany:m.links},spaStaffAvailability:{findMany:m.regular},spaStaffAvailabilityException:{findMany:m.exceptions},spaBooking:{findMany:m.bookings},spaServiceLocation:{findMany:m.locations}}}));
...
beforeEach(()=>{vi.clearAllMocks();m.store.mockResolvedValue("store");m.locations.mockResolvedValue([{id:"L",name:"位置"}]);m.staffFind.mockResolvedValue({id:"A"});m.staffList.mockResolvedValue([{id:"A",displayName:"甲"},{id:"B",displayName:"乙"}]);});
describe("service-provider relation",()=>{
 ...
 it("requires all services, excludes breaks and excludes existing bookings",async()=>{
  ...
  m.treatments.mockResolvedValue([{id:"T1",serviceMinutes:30,bufferMinutes:0,serviceLocations:[{serviceLocationId:"L"}],skills:[{skillId:"S1"}]},{id:"T2",serviceMinutes:30,bufferMinutes:0,serviceLocations:[{serviceLocationId:"L"}],skills:[{skillId:"S2"}]}]);
...
it("suggests only complete service intervals after a break and existing booking",async()=>{
 m.staffList.mockResolvedValue([{id:"A",displayName:"甲"}]);m.treatments.mockResolvedValue([{id:"T",serviceMinutes:45,bufferMinutes:15,serviceLocations:[{serviceLocationId:"L"}],skills:[]}]);m.links.mockResolvedValue([]);
...
describe("joint staff and location availability",()=>{
 beforeEach(()=>{m.staffList.mockResolvedValue([{id:"A",displayName:"甲"}]);m.treatments.mockResolvedValue([{id:"T",serviceMinutes:60,bufferMinutes:0,skills:[],serviceLocations:[{serviceLocationId:"L"}]}]);m.links.mockResolvedValue([]);m.regular.mockResolvedValue([{staffId:"A",startTime:"09:00",endTime:"18:00",isActive:true}]);m.exceptions.mockResolvedValue([]);});
 it("excludes a fully occupied location despite an available staff member and suggests after release",async()=>{
 m.bookings.mockResolvedValue([{serviceStaffId:"OTHER",serviceLocationId:"L",startTime:"12:00",endTime:"14:00"}]);
 const r=await getSpaAvailableProviders({date:"2026-09-11",startTime:"12:00",treatmentIds:["T"]});expect(r.success).toBe(true);if(!r.success)return;expect(r.people).toEqual([]);expect(r.locations).toEqual([]);expect(r.reason).toContain("位置");expect(r.suggestions[0]).toEqual({startTime:"14:00",endTime:"15:00"});
 });
 it("requires a common location across all selected services",async()=>{
 m.treatments.mockResolvedValue([{id:"T",serviceMinutes:30,bufferMinutes:0,skills:[],serviceLocations:[{serviceLocationId:"L"}]},{id:"T2",serviceMinutes:30,bufferMinutes:0,skills:[],serviceLocations:[{serviceLocationId:"L2"}]}]);m.locations.mockResolvedValue([{id:"L",name:"一"},{id:"L2",name:"二"}]);m.bookings.mockResolvedValue([]);
 const r=await getSpaAvailableProviders({date:"2026-09-11",startTime:"12:00",treatmentIds:["T","T2"]});expect(r).toMatchObject({success:true,people:[],locations:[],suggestions:[],setupHref:"/dashboard/spa-resources"});
 });
 it("returns only free locations and excludes the edited booking in the scoped query",async()=>{
 m.treatments.mockResolvedValue([{id:"T",serviceMinutes:60,bufferMinutes:0,skills:[],serviceLocations:[{serviceLocationId:"L"},{serviceLocationId:"L2"}]}]);m.locations.mockResolvedValue([{id:"L",name:"一"},{id:"L2",name:"二"}]);m.bookings.mockResolvedValue([{serviceStaffId:"OTHER",serviceLocationId:"L",startTime:"12:00",endTime:"13:00"}]);
 const r=await getSpaAvailableProviders({date:"2026-09-11",startTime:"12:00",treatmentIds:["T"],bookingId:"editing"});expect(r).toMatchObject({success:true,people:[{id:"A",name:"甲"}],locations:[{id:"L2",name:"二"}]});expect(m.bookings).toHaveBeenCalledWith(expect.objectContaining({where:expect.objectContaining({storeId:"store",id:{not:"editing"},status:{in:["PENDING","CONFIRMED"]}})}));
 });
});
