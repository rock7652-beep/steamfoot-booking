...
## LINE device failure and correction — 2026-09-16

- User supplied a real LINE screenshot at approximately 21:54 Taipei: onboarding displayed service unavailable, and the top brand mark was partly out of view. This is a failed device acceptance, not a password failure.
- Runtime logs at 13:54 UTC establish the sequence: token verification succeeded → need_onboarding → legacy helper created a new User/Customer → Account synchronization refused because the verified LINE Account already belonged to an existing User → a conflicting store identity link was nevertheless saved → the next exchange safely rejected the inconsistent ownership and again returned need_onboarding.
- Course onboarding now uses a separate serializable transaction. It reuses the verified central Account for a genuinely new store membership, creates new User/Account/Customer/store-link rows atomically when needed, and does not overwrite conflicting existing memberships. Name/phone equality is not used as authorization. Steamfoot/SPA retain their existing binding helper.
- Conflicting course identity records produce an actionable review message rather than a generic service failure or repeated registration form. Valid existing memberships require no writes.
- With the user's explicit approval, repaired only the failed attempt's new store link: linked it to the original LINE Account owner and removed its conflicting legacy Customer.userId reference. Retained the customer, the extra User, original Account, and all previous data. Read-only postcheck confirms active original owner, matching store/customer/identity scope, original Account retained, extra User retained, no bookings on the repaired member. No new customer was created for this repair.
- LIFF subpage header now displays the complete brand image in normal flow with safe-area padding. Onboarding terminal results blur the input and reset scroll so the keyboard's previous viewport offset does not leave the header above the visible area. Physical LINE navigation/keyboard still requires device recheck.
- Local regression: 89 tests passed across onboarding action, course onboarding service, verified identity resolver, exchange route and LIFF credentials; TypeScript and changed-file ESLint passed. Mock-based service tests prove call/rollback propagation behavior, not a new real LINE login.
- Actual LINE entry after correction remains pending user retest. No production database changes or merge.
