...
## 2026-09-26 本人收入補強（草稿）

基準 main `b0afa71d`。既有 CP01～CP16 已隨 #1094 發布；本批另增 CP17（教練我的收入）、CP18（會員端已連結店內人員的本人收入）。

- 共 18 題：會員 11 題（健康未開通 10 題）、教練 7 題；維持原入口、樣式與角色篩選。
- 一般會員不因此獲得收入查詢權限。須本店在職人員與帳號連結、開放本人查詢；有教練身分從我的工作進入，其餘從我的進入。
- 核對來源：`src/app/(customer)/book/course-portal-client.tsx`、`src/app/(customer)/book/income/page.tsx`、`src/server/services/course-personal-income.ts`、`src/lib/course-personal-income.ts`。
- 說明應領合計、待店長確認、調整中保留上次金額及歷史付款；LINE 提醒不等於實際到帳。
- 18 題均保留待實際角色登入驗收；未操作正式人員設定、顧客、預約或通知，新收入畫面截圖待安全測試工作階段。
