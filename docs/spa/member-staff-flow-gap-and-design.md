...
- 實機結果保存後，已依驗收者確認從會員畫面精確取消本節建立的預約。畫面保留 `CANCELLED` 歷史，9/17 10:00 重新出現在可選時段，`SPA 測試顧客` 可再次選擇，代表人員與 `床1` 的時段占用均已釋放。
- 取消後會員首頁顯示療程可使用 2 次、已預約 0 次、尚可預約 2 次；未扣次且沒有殘留 `RESERVED` 占用。既有會員、人員、排班、取消歷史及其他測試資料均未刪除。
