...
Before this convergence, the LINE identity write path was scattered across **8 separate sites** that all wrote some subset of `Customer.lineUserId` / `Account[line]` / `User`. The G5 series converged **4 of the 8**; the remaining 4 sites (1 already atomic + 3 non-atomic) are intentionally left in their pre-G5 shape (see §5 / §7 for rationale + remaining drift profile per site):

> Note: the 8th row (profile-merge) is a Customer-MERGE surface that touches LINE binding fields as a side effect of merging Customers — semantically distinct from the other 7 (which are LINE OAuth identity-write entry points). It was surfaced during the closeout-doc Codex review rounds; it was not part of the original G5 scope. Listed in the same table so a reader sweeping for "all LINE-binding write surfaces" finds it.
...
| **profile-merge LINE write path** (`src/server/services/customer-merge.ts` `mergePlaceholderCustomerIntoRealCustomer` + sibling merge helpers) | merge tx moves `Customer.lineUserId` between rows + calls `syncLineAccountForUser({...,tx})` as best-effort; the helper swallows Prisma errors and returns `{status:"error"}` instead of throwing | ⚠ tx + swallowed in-tx best-effort | ⏭ **NOT converged** — same shape today (calls at `customer-merge.ts:128-129, 163-164, 191-192, 232-233`); `Customer-ahead / Account-behind` drift window remains because the merge tx can commit even when in-tx Account-sync returns error (see §7 follow-ups) |
...
The G5 series stops the generators **on the converged 4 sites** (auth.ts Case A LINE / auth.ts Case B LINE / oauth-confirm `finalizeLineBind` / LIFF onboarding B4 precreated activation). The remaining 4 sites in the §1 inventory above — `auth.ts` Case C inline (already atomic, no drift to stop), LIFF 0-candidate `created_new`, webhook bind-code, and profile-merge LINE write path — are **NOT** converged by this series.

Of those 4, three are non-atomic and can still produce `Customer-ahead / Account-behind` drift when their post-tx / in-tx-but-swallowed Account sync fails: **LIFF 0-candidate `created_new`**, **webhook bind-code**, and **profile-merge** (where `syncLineAccountForUser` returns `{status:"error"}` instead of throwing, so the merge tx commits with Account potentially missing). The fourth (`auth.ts` Case C) is already atomic in its own `prisma.$transaction` so no drift can be generated there.

See §7 "Open convergence follow-ups" for the precise remaining drift profile and suggested convergence targets per non-atomic site. Do NOT read this doc as "all LINE drift sources have been eliminated" — read it as "the main OAuth signIn paths have been converged; three bounded non-atomic follow-ups remain documented (one of which — profile-merge — was surfaced during the closeout-doc Codex review and is a Customer-merge side-effect surface rather than an OAuth identity-write entry point)."
...
- `webhook bind-code` flow at `src/app/api/line/webhook/route.ts` — see §1 table footnote + §5 + §7. **D3 conversion is an open follow-up FOR THE `customer.userId !== null` SUBSET ONLY.** The `customer.userId === null` subset (precreated Customer + staff-issued bind code redeemed before activation) does NOT fit D3 and needs its own design — see §7 split-target breakdown. Do NOT assume webhook is D3-atomic when scoping future identity / drift work.
...
**Where enforced (scope-limited list):** within `bind-line-to-customer.ts` (D3 + D5 only — NOT `bindLineToCustomerInStore` 0-candidate branch, which still routes its Account write through the legacy `syncLineAccountForUser`), `auth-case-a-line-bind.ts`, `auth-case-b-line-activation.ts`, and the structured-log surfaces inside `oauth-confirm.ts` `finalizeLineBind`. The `logLineBindEvent` helper masks on its way out — callers can pass raw IDs to it. This list is exhaustive for the canonical-path masking guarantee; ANYTHING outside this list (notably `syncLineAccountForUser` itself, the webhook bind-code path, the oauth-confirm `resolveLineLogin` placeholder-bind branch, and all four `customer-merge.ts` call sites) is out of scope for this invariant — see the KNOWN GAP subsection below.
...
- **Webhook bind-code** is **NOT a single shape** — it splits into two subsets at runtime, because `generateLineBindingCode` (`src/server/actions/reminder.ts:439-476`) does NOT require `Customer.userId !== null`. Staff can issue a bind code to any not-yet-LINKED Customer, including precreated ones with no User row yet. The webhook redemption path (`src/app/api/line/webhook/route.ts:375-385`) explicitly branches on `if (customer.userId)`:
    - **Subset A — `Customer.userId !== null`** (returning customer with existing User redeems the code): does `prisma.customer.update + syncLineAccountForUser({userId, lineUserId})`. **This subset FITS D3** (`bindLineToExistingCustomerById`). A future PR could route it through D3 (with `oauthAccount` unset → 4-field minimal Account, matching `finalizeLineBind` byte-equivalent baseline) — small wiring PR analogous to PR-G5.2.a.
    - **Subset B — `Customer.userId === null`** (staff precreated a Customer + issued a bind code BEFORE the customer activated): does `prisma.customer.update` only, then logs `accountSyncStatus: "skipped_no_user"` and proceeds (deferring Account creation to the customer's later `/profile` activation flow). **This subset does NOT fit D3** — D3 requires `Customer.userId !== null` and returns `customer_has_no_user` otherwise; routing this subset through D3 would break the precreated-Customer bind-code flow. Convergence for this subset needs either a legacy/null-user-aware path that preserves today's "skip Account, defer to activation" semantic, OR a future D5/D6-style bind-code helper that atomically creates User+Account when the bind-code is redeemed by a precreated Customer. Smaller scope than the LIFF 0-candidate D6 case (no new Customer row needed; the row already exists from staff precreation), but still a design decision separate from the Subset A wiring.
...
|---|---|---|---|---|
| Webhook bind-code | `src/app/api/line/webhook/route.ts:361-385` (redemption) + `src/server/actions/reminder.ts:439-476` (`generateLineBindingCode`) | `prisma.customer.update` + IF `customer.userId !== null` THEN post-tx `syncLineAccountForUser` ELSE skip Account write (logs `skipped_no_user`) | Subset A (`userId !== null`): `Customer-ahead / Account-behind` if post-tx sync fails. Subset B (`userId === null`): no Account drift today (the path explicitly skips Account creation, deferring to later activation), but `Customer.lineUserId` is set without backing Account row until activation. | **SPLIT TARGET** — Subset A (`userId !== null`): **D3** (existing-customer helper; no `oauthAccount` since webhook has no OAuth tokens) — small wiring PR analogous to PR-G5.2.a. Subset B (`userId === null`): **NOT D3** (D3 returns `customer_has_no_user` for null-user Customers, which would break the precreated-Customer bind-code flow). Convergence options for Subset B: (a) preserve today's "skip Account, defer to activation" semantic as a separate null-user-aware path; (b) future D5/D6-style bind-code helper that atomically creates User+Account on redemption. |
...
> ⚠ Do NOT assume any of the three sites can be routed through D3 uniformly. Even the webhook bind-code path splits into two runtime subsets: **only the `Customer.userId !== null` subset fits D3** (D3's existing-User precondition); the `Customer.userId === null` subset (precreated Customer + staff-issued bind code redeemed before activation) does NOT fit D3 and needs its own design (preserve today's null-user skip-Account path OR a future D5/D6-style bind-code helper). LIFF 0-candidate needs a new D6-style create-and-bind helper. Profile-merge needs either a merge-aware variant or a behavioural change to `syncLineAccountForUser` error handling. **Four distinct convergence designs minimum** (webhook Subset A + webhook Subset B + LIFF 0-candidate + profile-merge), not one — and converging Subset A without Subset B would actively break the precreated-Customer bind-code flow.
