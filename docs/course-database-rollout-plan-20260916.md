...
## 2026/09/20 最新完整演練（取代下方舊份數，歷史保留）

完整順序與原始 SQL SHA-256 見 `course-full-migration-rehearsal-20260920.json`，共15份。前13份後依序加入體驗收款／出席分離及第二批課程、人員資格遷移。不能依檔名時間跨目錄排序。

本機 PostgreSQL 17.6、無網路容器、合成相依資料，實際通過全部套用、末段故意失敗完整回滾、缺相依拒絕、部分結構拒絕及重放拒絕；驗證舊資料未變、CARD 預設、資格待補預設、課型／狀態約束，以及13張課程表的瀏覽器角色隔離。這不是正式庫副本還原或網頁交易驗收。

第14份 SQL 自带 BEGIN/COMMIT。演練組裝時只移除經明確檢查的一對外層交易包裝，將所有語句放在單一交易；原 SQL 未修改。正式執行包必須同樣審查交易邊界，不能直接串接導致中途 COMMIT。演練工具只接受固定本機無網路容器，不接受正式資料庫 URL。

發布前仍須：正式最新 schema／依賴差異再核對、可用備份與實際恢復證據、遷移執行包及版本配對審查、明確正式授權。既有備份完成紀錄不等於恢复演練。尚未正式執行任何遷移。

失敗若在同一交易中則完整回滾；成功且已有新交易後，不以刪表退版。先阻止新增課程寫入，保留交易與欄位，採相容程式回退或前向修復。正式發布自動遷移白名單本次未擴大。

## 2026-09-17 協商退款追加（隔離遷移已完成）

使用者於 2026-09-17 明確授權具體測試專案後，第12份遷移經 MCP 成功套用並核對。原退款筆數、金額與外鍵／重送唯一性保留；舊列 method 為 OTHER，不猜測原方式。正式專案未修改。

第12份依序接在 MessageLog 課程連結遷移後：`supabase/migrations/20260917081039_course_negotiated_refund.sql`。新增退款 method（舊列預設OTHER，不回填猜測方式）、退款收回額度允許0、購買單可有多筆退款，保留同店requestKey唯一性。累計金額上限由同店鎖內的交易服務檢查。

十二份遷移已在無網路的本機 PostgreSQL 17.6 容器整批套用、故意失敗回滾及重放拒絕演練，另驗證方法枚舉、負額度拒絕、零額度補登與重送鍵唯一性。這是合成資料演練，不是真實交易驗收。

回復優先停用新退款寫入並修正程式，保留退款資料與新增欄位。已有補登退款後不能直接恢復purchaseId唯一限制或刪除method欄位；不得刪退款列以讓舊約束通過。正式資料庫仍未執行任何更新。
