...
1. **`storeId` 是 required, trusted context**：呼叫者必須從**可驗證**的可信來源取得（不可從 URL query / form field / 不受驗證的 raw cookie / 任何 client payload）。**可信來源**只限以下三類，**且 source 本身必須通過 server-side 驗證後** caller 才能取值：
   ...
   - **NextAuth signIn callback 內 `targetStoreId`（Case B 來源；⚠ 非 signed/trusted authorization context — Codex round 12 P2）**：實際路徑是 `src/lib/auth.ts` 透過 `resolveStoreFromOAuthCookie` 讀取 client-set `oauth-store-slug` cookie 後解析得來。**此 cookie 不是 NextAuth signed JWT**，使用者可在 DevTools / curl 自由設定值。`targetStoreId` 只能視為 **routing hint / store candidate**（決定「callback 要嘗試操作哪家店」的提示），**不可**作為授權依據。真正的 store authorization 邊界由 activation helper 內部建立 — 見 §5.3.3 Pre-write checklist step 3（`customer.storeId === storeId` 不符 → `store_mismatch` + 0 writes）。
...
```ts
activatePrecreatedCustomerWithLine({
  storeId,           // required, store candidate from caller-side trust-source list
                     // (同 §5.3 rule 1 三類). ⚠ Case B 來源 = auth.ts
                     // resolveStoreFromOAuthCookie 讀 client-set `oauth-store-slug`
                     // cookie — 只是 routing hint，不是 signed JWT；真正 authority
                     // 由 helper step 3 (customer.storeId === storeId) 建立
                     // (Codex round 12 P2)
...
| - | --- | --- | --- | --- |
| **1** | 接收 caller 提供的 `storeId` 作為 **store candidate**（**不**作為 authorization 結論）。⚠ Case B 來源是 `auth.ts` 透過 `resolveStoreFromOAuthCookie` 讀取 **client-set `oauth-store-slug` cookie** 解析得來 — **不是** NextAuth signed JWT、**不是** server-side trusted authorization context（Codex round 12 P2）。helper **不可**因為值「來自 callback 流程」就跳過後續 store check；真正的 store authorization 由 step 3 `customer.storeId === storeId` 建立。本 step 1 **不**呼叫 §5.3.1 cookie integrity（Case B 不用 `oauth_line_session` cookie），但**也不**把 `oauth-store-slug` cookie 當 trusted；它的角色限定為「callback 想操作哪家店的提示」。 | caller | n/a（純收值，無 verification 在此 step） | 0 |
...
> | storeId 來源（**注意：caller 端取得 = "store candidate"，不是 authorization 結論；真正 authority 在 helper step 3**） | webhook resolveStore（DB-resolved 真 trust）/ signed+verified oauth_line_session cookie（§5.3.1 通過後才 trust） | ⚠ NextAuth signIn callback 內 `targetStoreId` — **client-set `oauth-store-slug` cookie**（透過 `resolveStoreFromOAuthCookie` 解析；非 signed JWT，只是 routing hint，Codex round 12 P2） |
...
- **Case B**（customer 存在但無 userId）：本質是「為 backend-pre-created Customer 啟用 NextAuth User + 綁 LINE」。**必須 wire §5.3.3 的 activation helper `activatePrecreatedCustomerWithLine({ storeId, customerId, lineUserId, lineName, oauthProfile, oauthAccount })`**，其中 `storeId` 從 NextAuth signIn callback 內 `targetStoreId` 傳入。⚠ **`targetStoreId` 目前不是 signed JWT session 來源**，而是由 `src/lib/auth.ts` 透過 `resolveStoreFromOAuthCookie` 讀取 **client-set `oauth-store-slug` cookie** 解析得來（Codex round 12 P2）。此值只能視為 **routing hint / store candidate**，不可視為 trusted authorization context；PR-G5.5 不得因為它來自 callback 流程就跳過 store authorization 思考。真正的安全邊界必須由 activation helper 內部 load Customer 後檢查 **`customer.storeId === storeId`**（§5.3.3 step 3）建立；若 mismatch，**必須** return `store_mismatch` 且 **0 writes**（Customer / User / Account 全 0 byte）。`oauthProfile` 從 NextAuth signIn callback 的 `user` 參數萃取 `{ email, image, name }`（⚠ `name` 不會被寫進 `User.name`，僅作 Customer.lineName fallback；`User.name` 用 `customer.name`，baseline 行 624 — Codex round 10 P2）；`oauthAccount` 從 signIn callback 的 `account` 參數萃取 **9 個欄位** `{ provider, providerAccountId, type, access_token, refresh_token, id_token, expires_at, scope, token_type }`（⚠ **無 `session_state`**，baseline auth.ts Case B lines 634-647 完全沒寫此欄位 — 加上去 = silently 擴增 Account row、違反 byte-equivalent，Codex round 10 P2；列出的 9 個欄位 null/undefined 也必須原值傳遞，不可 silently drop — Codex round 9 P2）。**禁止** wire 至 existing-user helper `bindLineToExistingCustomerById` — 那條 helper 的 step 4 會在 `customer.userId === null` 立刻 abort，把 Case B 啟用流程整個打斷（Codex round 5 P1 點出的 regression）。activation helper 內部會自驗 `customer.storeId === storeId` + `customer.userId === null` + `customer.lineUserId === null`，Case B caller 不需要在外面重複防衛。
