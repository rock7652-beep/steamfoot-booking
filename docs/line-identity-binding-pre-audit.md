...
activatePrecreatedCustomerWithLine({
  ...
  // ⚠ OAuth metadata preservation（Codex round 9 P2）— PR-G5.5 必須**原封不動**
  //   把 auth.ts Case B 現有寫入的 OAuth 欄位轉交 helper，refactor 才能符合
  //   「byte-equivalent except transactional grouping」的合約。漏帶任何欄位
  //   即等於靜默改變 User / Account 行為，違反 PR-G5.5 refactor-only 約定。
  oauthProfile: {
    email,           // → User.email（Case B 現行寫入；NULL 也要原值傳；不可刪欄位）
    image,           // → User.image
    name,            // → User.name（與 lineName 不同來源；NextAuth profile().name，
                     //              Case B 用「LINE 用戶」或 OAuth name fallback）
  },
  oauthAccount: {
    provider,            // 預期 "line"（PR-G5.5 範圍只有 LINE provider，但欄位仍由 caller 帶）
    providerAccountId,   // == lineUserId；冗餘但保留以對齊 NextAuth Account schema
    type,                // 預期 "oauth"
    access_token,        // 任一 token 欄位 null/undefined 也必須原值傳遞（不可省略 key）；
    refresh_token,       //   helper 不得 silently drop（refactor byte-equal 規則）
    id_token,
    expires_at,
    scope,
    token_type,
...
**OAuth 欄位保留規則（Codex round 9 P2 必入測試）**：

- PR-G5.5 是 **refactor-only**：把 Case B 既有「`User.create` + `Account.create` + `Customer.update` 三件分次 top-level write」整併進單一 `$transaction`，**除了 transactional grouping 之外的所有 column 值必須 byte-equivalent**（含 `User.email` / `User.image` / `User.name`、`Account.provider` / `providerAccountId` / `type` / `access_token` / `refresh_token` / `id_token` / `expires_at` / `scope` / `token_type` / `session_state`）。
- Helper **不得** silently drop 任何 OAuth account 欄位，**即使值為 null/undefined** 也必須把該 key 顯式傳給 `prisma.account.create`（與既有 `auth.ts` Case B 行為一致）。
- 若未來真有業務需求要改動 OAuth metadata 寫入行為，**另開 PR train**、明確 PR description 寫「intentional change vs PR-G5.5 byte-equivalent baseline」；**不**在 PR-G5.5 內偷渡。
...
| **6** | 在單一 `$transaction` (Serializable) 內 **原子地**: (a) `prisma.user.create` 建 User，欄位 = `{ name: oauthProfile.name, email: oauthProfile.email, image: oauthProfile.image, role: "CUSTOMER", status: "ACTIVE" }`（**所有 OAuth profile 欄位必須原值傳入；null/undefined 也照傳，不可 silently drop** — Codex round 9 P2）；(b) `prisma.account.create` 建 Account，欄位 = `{ userId: newUser.id, provider: oauthAccount.provider ?? "line", providerAccountId: oauthAccount.providerAccountId ?? lineUserId, type: oauthAccount.type ?? "oauth", access_token: oauthAccount.access_token, refresh_token: oauthAccount.refresh_token, id_token: oauthAccount.id_token, expires_at: oauthAccount.expires_at, scope: oauthAccount.scope, token_type: oauthAccount.token_type, session_state: oauthAccount.session_state }`（**所有 token 欄位必須原值傳入，與既有 auth.ts Case B byte-equivalent**）；(c) `prisma.customer.update` 寫 `{ userId: newUser.id, lineUserId, lineName, lineLinkStatus: "LINKED", lineLinkedAt: now }`；(d) AuditLog 寫一筆「Case B activation」row；任一 throw 整組 rollback（A3 atomicity） | helper 內部 | `unique_conflict`（P2002）/ Prisma throw 重拋 | tx rollback → 0 |
...
- **Case B**（customer 存在但無 userId）：本質是「為 backend-pre-created Customer 啟用 NextAuth User + 綁 LINE」。**必須 wire §5.3.3 的 activation helper `activatePrecreatedCustomerWithLine({ storeId, customerId, lineUserId, lineName, oauthProfile, oauthAccount })`**，其中 `storeId` 從 NextAuth signIn 階段已 resolve 的 `targetStoreId` 傳入（trusted source — 不是來自 user input、不經 cookie）；`oauthProfile` 從 NextAuth signIn callback 的 `user` 參數萃取 `{ email, image, name }`；`oauthAccount` 從 signIn callback 的 `account` 參數萃取 `{ provider, providerAccountId, type, access_token, refresh_token, id_token, expires_at, scope, token_type, session_state }`（**所有欄位 null/undefined 也必須原值傳遞，不可 silently drop — Codex round 9 P2 byte-equivalent 規則**）。**禁止** wire 至 existing-user helper `bindLineToExistingCustomerById` — 那條 helper 的 step 4 會在 `customer.userId === null` 立刻 abort，把 Case B 啟用流程整個打斷（Codex round 5 P1 點出的 regression）。activation helper 內部會自驗 `customer.storeId === storeId` + `customer.userId === null` + `customer.lineUserId === null`，Case B caller 不需要在外面重複防衛。
...
| **PR-G5.5** | auth.ts Case B 收斂到 **`activatePrecreatedCustomerWithLine`（§5.3.3 activation helper，唯一 wire 此 helper 的 caller）**；**禁止** wire 至 `bindLineToExistingCustomerById`（會被 step 4 `userId === null` 擋下、首次 LINE 啟用流程整個壞掉，Codex round 5 P1）。Refactor only — 把 Case B 既有的「User.create + Account.create + Customer.update 三件分次 top-level write」整併進 helper 單一 `$transaction`；**OAuth metadata 必須完整轉交 helper**：caller 從 NextAuth signIn callback 萃取 `oauthProfile = { email, image, name }` 與 `oauthAccount = { provider, providerAccountId, type, access_token, refresh_token, id_token, expires_at, scope, token_type, session_state }` 並原值傳給 helper（**漏帶任何欄位 = 違反 byte-equivalent 合約**，Codex round 9 P2） | S | Golden-output tests：(a) 既有 Case B input 經 refactor 後 DB 寫入**逐欄位 byte-equal**，**含**：`User.{email, image, name}` + `Account.{provider, providerAccountId, type, access_token, refresh_token, id_token, expires_at, scope, token_type, session_state}` + `Customer.{userId, lineUserId, lineName, lineLinkStatus, lineLinkedAt}`（除了 timestamp 與 tx grouping 之外，不應有任何欄位差異）；(b) regression：spy 確認 Case B 路徑**不**呼叫 `bindLineToExistingCustomerById`；(c) regression：null/undefined OAuth token 欄位必須照寫進 Account row，**不**被 helper silently drop |
...
| `activatePrecreatedCustomerWithLine`（§5.3.3，Case B 啟用專用） | 全部 status 分支：`activated` / `store_mismatch` / `customer_already_has_user` / `customer_already_linked_to_other_line` / `unique_conflict`（P2002）+ **`activated` happy path test**：傳入完整 `{ storeId, customerId: <staff-precreated>, lineUserId, lineName, oauthProfile, oauthAccount }` → 在單一 `$transaction` 內 `prisma.user.create` × 1（assert 欄位 = `{ name: oauthProfile.name, email: oauthProfile.email, image: oauthProfile.image, role: "CUSTOMER", status: "ACTIVE" }`，**OAuth profile 全 3 欄位 byte-asserted**）+ `prisma.account.create` × 1（assert 欄位 = `{ userId: newUser.id, provider: oauthAccount.provider, providerAccountId: oauthAccount.providerAccountId, type: oauthAccount.type, access_token: oauthAccount.access_token, refresh_token: oauthAccount.refresh_token, id_token: oauthAccount.id_token, expires_at: oauthAccount.expires_at, scope: oauthAccount.scope, token_type: oauthAccount.token_type, session_state: oauthAccount.session_state }`，**全 10 個 OAuth/token 欄位 byte-asserted，含 null/undefined 值的欄位也要被寫入，不可被 helper silently drop** — Codex round 9 P2）+ `prisma.customer.update`（userId / lineUserId / lineLinkStatus / lineLinkedAt / lineName）+ `prisma.auditLog.create` × 1；+ **OAuth byte-equivalent regression test**：對同一輸入跑兩次 happy path —（i）refactor 後 helper、（ii）refactor 前 auth.ts Case B inline 寫法（測試輔助用，僅在 test fixture 內 reproduce），對 User / Account 兩個 row 逐欄位 deep-equal，**含**所有 token 欄位 + null/undefined；任何欄位有差就 fail，PR-G5.5 refactor-only 合約被 silently 改動會立刻被擋下；+ **null token preservation test**：傳入 `oauthAccount.{access_token, refresh_token, id_token, scope, token_type, session_state}` 全為 `null` → spy 確認 `prisma.account.create` 收到的 data 物件中**這些 key 全部存在且值為 `null`**（而非 key 被省略）+ **atomicity test**：mock 第 (b) `prisma.account.create` 拋錯 → 整 `$transaction` rollback，**不殘留** orphan User 行（Customer.userId / lineUserId 都不得寫入）+ **`customer_already_has_user` pre-write semantics test**：傳入 `customerId` 指向已有 `userId` 的 Customer → return `customer_already_has_user`，spy 確認 user.create / account.create / customer.update / auditLog.create 全 **0 次**；錯誤訊息暗示 caller 應改 wire `bindLineToExistingCustomerById`（防止 Case B 被誤接至 existing-user helper 的對稱保險）+ **`store_mismatch` pre-write semantics test**：同 existing-user helper 形式 + **`customer_already_linked_to_other_line` pre-write semantics test**：Customer.lineUserId 已被別的 LINE 占用 → 0 byte 寫入（防 hijack） |
...
| **auth.ts Case B activation via `activatePrecreatedCustomerWithLine`**：staff 後台先建 `Customer { userId: null, lineUserId: null }` → 顧客首次 LINE OAuth 進站 → NextAuth signIn callback 命中該 Customer + 呼叫 activation helper（**含完整 `oauthProfile` + `oauthAccount` 參數**）→ 單一 `$transaction` 內建 User（含 OAuth `email` / `image` / `name`）+ Account[line]（含完整 `provider` / `providerAccountId` / `type` / `access_token` / `refresh_token` / `id_token` / `expires_at` / `scope` / `token_type` / `session_state`，**null/undefined 也要寫入 row，不可被 helper silently drop**）+ 更新 Customer (userId/lineUserId/lineLinkStatus/lineLinkedAt/lineName) → DB 寫入序列與既有 top-level 三件分次寫的 refactor-前 baseline **逐欄位 byte-equal**（含 User OAuth 3 欄位 + Account 10 OAuth/token 欄位 + Customer 5 欄位；除了改用單一 tx 取代分次 commit、不應有其他差異 — Codex round 9 P2 byte-equivalent 合約） | G5.5 |
