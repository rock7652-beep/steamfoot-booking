...
- 使用者截圖確認 LINE 登入、補基本資料後返回購買頁、送單成功；測試訂單 TXN-20260916-001，金額 3,000、後四碼 1111，顯示待店家確認。
- 複製付款資訊顯示「已複製」，尚未核對貼上內容。
- 發現完成頁「回到我的方案」連到僅接受 LIFF 的入口；改連到既有支援網頁登入的同店 /my-plans，首頁改連到 /book，與網頁購買完成頁一致。未更動認證保護。
- 測試店 LINE 連結為 staging-staging 測試值，僅見加好友 QR 畫面，不能視為有效店家聯絡驗收。
- 店長核帳、方案實際入帳與修正後返回操作仍待驗收；前段未建立訂單的記錄為先前狀態，目前使用者已建立上述測試申請。
