...
## 目前狀態：2026-09-26 增量盤點（草稿）

- 已核對 main `64d30d3850fad31e11f87811750a42128e07919d..b0afa71dba9f158ada393085d2247968e52ac03c`，8 筆提交、84 個變更檔案。包含 #1097、#1098、#1100～#1105；#1098 已正式發布，不重複新增原有 145 題。本次只建立新草稿，續查基準記於 `operation-guide-audit-state.json`。
- 新增 **5 題**：後台 C142（手動月結 LINE 通知）、E13（蒸足月結服務收入／租金分列）、E14（租金約定）；前台 CP17（教練本人收入）、CP18（已連結人員的會員端收入入口）。
- 更新後台 **12 題**：C105、C112、C121、C122、C124、C125、C126、C128、C134、C138、C139、C140。同步購買方案與現金抽屜、退款／作廢、已預約額度明細、月結篩選／確認／查看權限及歷史付款的文字與搜尋詞。
- 草稿後台 **148 題**（課程適用 54、SPA 適用 63，蒸足新增 2）；課程前台另計 **18 題**（會員 11，健康未開通 10；教練 7）。依原有模組／角色／權限／功能篩選，不把課程文章複製給其他模組，不增加 UI 入口。
- 累計待實際操作驗收 **103 題**：後台 85＋前台 18。既有 98＋新 5；本批更新 12 題都已在待驗清單，去重。本批直接影響 17 題皆未以測試替代操作驗收。
- 新版課程「每月收入結算」不再顯示待處理／已結清篩選、新增利潤／授課費付款控制；只在「查看權限」開放本人查詢。舊計算開關儲存值不因查詢開關被改寫。保留的函式/API 不等於可見入口，不教使用者繞過畫面操作。
- 月結通知需確認目前版本、開放本人查詢、LINE 功能與有效綁定；同版去重、安全重試期限與額度限制另述。提醒不代表付款，預覽模式封鎖發送。本輪不呼叫發送動作、不修改設定。
- 蒸足租金以完整租期顯示，不從服務收入扣抵、不重複累加同一期、不自動收付款；調整／停止接續完整租期，停用人員不等於停止約定。
- 實際操作及新截圖待已確認資料隔離、通知封鎖的登入工作階段。維持綠金風格與既有唯一頂欄入口，只改文章、測試及文件，沒有改預約、共卡、扣點或收款規則。尚未合併的需求不列作 main 功能。
- 本機指南相關 6 檔 **37 項**及功能相關 13 檔 **118 項測試通過**；部署／CI 最終結果記於本批 PR。程式核對、HTTP 成功與部署就緒均不表示登入後驗收通過。

### 核對來源與待驗收重點

| 文章 | 實作來源 | 隔離環境待驗 |
| --- | --- | --- |
| C138～C140、C142、CP17～CP18 | course-monthly.tsx、course-monthly-client.tsx、course-monthly-notifications.tsx、actions/services/course-monthly-notification.ts、book/income/page.tsx、services/course-personal-income.ts | 月份／類別／姓名篩選、確認版本、開放本人查詢、本人限定、通知預覽與封鎖，絕不發真通知 |
| C105、C112、C121、C124 | course-transaction-actions.tsx、course-assignment-payment.tsx、actions/course-checkout-status.ts、member-workspace.tsx | 原交易核對、退款試算不自動填值、停用提示、購買入口、抽屜缺少／關閉／重查；不得動正式交易 |
| C122 | revenue/_components/course-fees.tsx、revenue/_components/course-fee-payment-button.tsx、service-fee-calculator/course-monthly-client.tsx | 區分收入頁授課費與月結歷史付款，不尋找已移除的新付款入口 |
| C134 | card-browser.tsx、card-reservations.tsx、actions/course-card-reservations.ts | 剩餘／已預約／可用、共卡共同餘額、20 筆分頁、權限限定可見明細 |
| E13、E14 | steamfoot-monthly.tsx、queries/steamfoot-monthly.ts、staff/[id]/rent/rent-form.tsx、actions/steamfoot-rent.ts、lib/steamfoot-rent.ts | 收入與租金分列、同一期不重複累加、完整租期調整／停止、唯讀與店主人員限制 |

來源路徑以各後台文章 sources 與前台覆蓋表為準；上表檔名為閱讀索引。本批必要後台／前台新截圖仍未製作，不拿舊畫面當成新版證據。

## 2026-09-25 增量盤點結果（歷史狀態）

發布前補核：使用者已授權 #1098 正式發布；同步 main #1097（250671fb），依課程月結元件、本人收入 service／page 修正 C138 篩選說明及 C139 人員查詢開關、入口、已確認金額與付款語意，新增「本人收入／我的收入」搜尋詞。不另加題、不開啟任何店家設定、不修改實際帳務；仍保留 98 題待操作驗收。本段取代本批「不自動正式發布」的交付狀態，歷史核對紀錄保留。
