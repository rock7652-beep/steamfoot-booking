...
- 移除左側選單、預約頁標題、預約明細中的重複按鈕；頂欄只保留「？操作指南」。
- 桌機使用非模態面板，保留380px教學空間；測試時段後台 RightSheet 頂端避開頂欄，開啟教學後右側也預留空間。CSS 僅作用在啟用指南的預覽容器。手機仍使用原生模態底部面板。
- 預約明細只登記教學情境，不掛載第二個入口；目前文章、搜尋與展開狀態保留，手動點「查看此頁相關問題」才回列表。
- 文章切換／返回重設教學捲動，不改原操作區位置；精簡第三步，結果提示集中於完成確認。
- 搜尋標示「預約操作教學」、結果加功能名稱，避免「提醒」誤認為LINE通知教學。
- 本輪需實機確認桌機並排、手機375/390與預約明細／其他彈窗層級。不將靜態與元件測試當成實際視覺通過。
