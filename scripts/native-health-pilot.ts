...
async function snapshot() {
  ...
  const [entitlements, records] = await Promise.all([
    prisma.storeFeatureEntitlement.findMany({
      ...
      select: { storeId: true, status: true, source: true, startsAt: true, expiresAt: true, note: true },
    }),
    prisma.customerHealthRecord.findMany({
      where: { source: "HEALTHFLOW" },
      select: { id: true, storeId: true, customerId: true, sourceRecordId: true, customer: { select: { storeId: true } } },
    }),
  ]);

  if (records.length !== 431) {
    throw new Error(`正式 HEALTHFLOW 紀錄應為 431 筆，實際 ${records.length} 筆，停止`);
  }
  if (records.some((record) => !record.sourceRecordId)) {
    throw new Error("存在缺少來源 ID 的 HEALTHFLOW 紀錄，停止");
  }
  if (new Set(records.map((record) => record.sourceRecordId)).size !== 431) {
  ...
  if (crossStore.length > 0) {
    throw new Error(`發現 ${crossStore.length} 筆跨店健康紀錄，停止`);
  }
  ...
  const counts = Object.fromEntries(
    stores.map((store) => [
      store.slug,
      {
        ...
        customers: new Set(
          records.filter((record) => record.storeId === store.id).map((record) => record.customerId),
        ).size,
        entitlement: entitlements.find((item) => item.storeId === store.id) ?? null,
      },
    ]),
  );
...
async function main() {
    ...
    for (const slug of CONTROLLED_SLUGS) {
      ...
      await tx.storeFeatureEntitlement.upsert({
        ...
        create: {
          ...
          status,
          source: "HQ_OVERRIDE",
          note: slug === TARGET_SLUG
            ? "2026-08-24 原生健康功能單店小範圍驗收"
            : "2026-08-24 原生健康功能分店維持關閉",
        },
        update: {
          status,
          source: "HQ_OVERRIDE",
  ...
  for (const slug of CONTROLLED_SLUGS) {
    ...
    if (!entitlement || entitlement.status !== expected || entitlement.source !== "HQ_OVERRIDE") {
      throw new Error(`${slug} 開關驗證失敗`);
    }
  }
  console.log(JSON.stringify({ result: "verified", after }, null, 2));
}
