/**
 * scripts/repair-line-mismatch-pr-f2-1.ts — DRY RUN default, single-record only
 *
 * PR-F2.1: First per-record repair of a `needs_customer_merge` LINE
 * account-mismatch in store=zhubei. Strict conformance to
 * docs/pr-f2-line-mismatch-repair-plan.md (PR #221, merged).
 *
 * ## Target record
 *
 * Picked from PR-F1.2 audit (`scripts/diagnose-line-mismatch-repair-audit.ts
 * --store=zhubei`) as the FIRST `needs_customer_merge` row:
 *   - earliest customer.user.createdAt of all 3 mismatch records
 *   - smallest footprint of the two `needs_customer_merge` candidates
 *     (2 bookings, 2 transactions, 1 active wallet) → smallest blast
 *     radius for the very first repair PR
 *   - PR-F1.2 classify() said: primary=customer / shell=account /
 *     canReassignSafely=true / account.customer=shell_merge_deactivate /
 *     recommendation=needs_customer_merge
 *
 * The second `needs_customer_merge` row (bigger footprint) is OUT OF SCOPE
 * for this PR; it gets its own PR-F2.2 with its own dry-run + reviewer cycle.
 * The third row (`needs_manual_business_check`) is OUT OF SCOPE for the
 * entire PR-F2 series until store-owner + business sign-off (per PR-F2.0 §5).
 *
 * ## Safety
 *
 ...
 *     npx tsx scripts/repair-line-mismatch-pr-f2-1.ts
 *
 *   # APPLY — must be done in a SEPARATE invocation after reviewer sign-off
 *   DATABASE_URL="$DIRECT_URL?connection_limit=1" \
 *     OPERATOR_USER_ID="<reviewer-User.id>" \
 *     npx tsx scripts/repair-line-mismatch-pr-f2-1.ts --apply
 *
 * This PR (PR-F2.1) only commits the script. It does NOT execute --apply
 * against production.
 */
...
// Filled by /tmp/pr-f2-1-fill.ts (one-shot DB fetch); see commit message.
// Raw phone / raw lineUserId never leave this file; printRecords use masked
// helpers. Reviewer audit reads constants directly from PR diff.
const CANONICAL_CUSTOMER_ID   = "cmojgmvsr0001l104lrt6a0fa";
const CANONICAL_USER_ID       = "cmns9rx9i0000l804mei64rwk";
const PLACEHOLDER_CUSTOMER_ID = "cmpak3szh0002if04v51ernp5";
const PLACEHOLDER_USER_ID     = "cmpak3sza0000if04sve6tdbv";
const LINE_ACCOUNT_ID         = "cmpak3szp0004if048fngckkn";
const CANONICAL_STORE_ID      = "e182e256-98ca-4c78-970b-d4b118066c51";
const CANONICAL_STORE_SLUG    = "zhubei";
const CANONICAL_PHONE         = "0919345865";
const LINE_USER_ID            = "U0edfbbe8f49df7a86a9eddd3e85c0450";
  ...
  if (!APPLY) {
    console.log(
      "\nDRY RUN — 沒有寫入。To apply, re-run after reviewer sign-off with:\n" +
        "  OPERATOR_USER_ID=<reviewer-User.id> npx tsx scripts/repair-line-mismatch-pr-f2-1.ts --apply",
    );
    return;
  }
...
const runDirectly =
  !!process.argv[1] && /repair-line-mismatch-pr-f2-1/.test(process.argv[1]);

if (runDirectly) {
  main()
    .catch((err) => {
      console.error("[repair-line-mismatch-pr-f2-1] failed:", err);
      process.exit(1);
    })
    .finally(async () => {
