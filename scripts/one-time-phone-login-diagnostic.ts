...
async function main() {
  const [customers, users, links, stores] = await Promise.all([
    prisma.customer.findMany({ select: { id:true, storeId:true, name:true, phone:true, userId:true, lineUserId:true, lineLinkStatus:true, mergedIntoCustomerId:true, planWallets:{where:{status:"ACTIVE",remainingSessions:{gt:0}},select:{id:true,remainingSessions:true,expiryDate:true,status:true}} } }),
