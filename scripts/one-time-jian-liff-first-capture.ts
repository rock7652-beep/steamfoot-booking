/** One-time owner-authorized LIFF first-capture repair for a single customer. */
...
async function main() {
  const result = await prisma.$transaction(async (tx) => {
    ...
    const [customer, user, actor, storeCustomers, links, accounts, pending] = await Promise.all([
      ...
      tx.customerIdentityLink.findMany({where:{OR:[{customerId:EXPECTED.customerId},{userId:EXPECTED.userId}],provider:"line"},select:{id:true},take:2}),
      tx.account.findMany({where:{userId:EXPECTED.userId,provider:"line"},select:{id:true},take:2}),
    ...
    if (links.length || accounts.length) throw new Error("LOGIN_IDENTITY_ALREADY_PRESENT");
    ...
    await tx.auditLog.create({data:{
      ...
      targetId:request.id,
      action:"AUTHORIZE_LIFF_LOGIN_FIRST_CAPTURE",
      beforeJson:{customerId:EXPECTED.customerId,loginIdentityPresent:false},
      afterJson:{requestId:request.id,status:"PENDING_CAPTURE",reason:REASON,expiresAt:request.expiresAt.toISOString(),scope:"single-customer"},
    }});
